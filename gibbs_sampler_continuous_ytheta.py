...
class Sampler:
    '''
        ...
        Collapsed noise processes around y.
        Sample y
        Sample theta
    ...
    def init_tc(self, tc=None):
        ...
        self.ATmtc_t = np.power(self.time_weights[0], self.T - np.arange(self.T) - 1.)
    ...
    def init_y(self):
        '''
            Initialise y given current theta and n_t
                Just a big gaussian
        '''
        ...
        B_Wmu = (self.random_network.get_network_features_combined(self.theta).T*self.time_weights[1, self.tc]).T
        
        # Compute the mean
        # mu = A m_tc_minus + B \sum_r W_r mu_r(\theta_r) + P (A^{T-tc})^T (A^{T-tc}PA^{T-tc}^T + \sigma_tc_plus)^{-1} (n_t - m_tc_plus - A (A m_tc_minus + B \sum_r W_r mu_r(\theta_r)))
        ...
        # cov = (P^-1 + A^T-tc \Sigma_tc_plus^-1 A^T-tc)^-1
    ...
    def plot_likelihood_variation_twoangles(self, num_points=100, should_plot=True, should_return=False, n=0, t=0):
        ...
        def loglike_theta_fct(thetas, (datapoint, rn, theta_mu, theta_kappa, sampled_feature_index, covariance_fixed_contrib_inv, B, sampler, n, t, ATmtc)):
            '''
                Compute the loglikelihood of: theta_r | n_t theta_r'
            '''
            ...
            return theta_kappa*np.cos(thetas[sampled_feature_index] - theta_mu) - 0.5*np.dot(like_mean, np.dot(covariance_fixed_contrib_inv, like_mean))
        ...
        covariance_fixed_contrib_inv = np.linalg.inv(self.Y_covariance[t])
        sampled_feature_index = 0
        params = (self.Y[n], self.random_network, self.theta_gamma, self.theta_kappa, sampled_feature_index, covariance_fixed_contrib_inv, self.time_weights[1, t], self, n, t, ATmtc)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--M', default=64, help='Dimensionality of data/memory')
