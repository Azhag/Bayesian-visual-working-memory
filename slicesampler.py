...
class SliceSampler:
    def __init__(self, step_out=True):
        self.step_out = step_out
        
    
    
    def sample(self, N, x_initial, loglike_fct, burn=100, widths=1., last_loglikehood=None, loglike_fct_params=None):
    ...
    def sample_1D(self, N, x_initial, loglike_fct, burn=100, widths=1., last_loglikehood=None, loglike_fct_params=None, bounds=(-np.inf, np.inf), thinning=1):
...
if __name__ == '__main__':
    ...
    if True:
        ...
        # Get samples
        slicesampler = SliceSampler(step_out = True)
        samples, last_llh = slicesampler.sample_1D(50000, -1000., loglike_fct, burn=100, widths=1.0, loglike_fct_params=loglike_fct_params)
    ...
    if False:
        # Try with Von Mises
        loglike_theta_fct = lambda x, (mu, kappa): kappa*np.cos(x - mu)
        loglike_fct_params = [0.0, 0.01]
        
        # Get samples
        slicesampler = SliceSampler(step_out = True)
        samples2, last_llh = slicesampler.sample_1D(50000, np.random.rand(), loglike_theta_fct, burn=100, widths=1.0, loglike_fct_params=loglike_fct_params, bounds=(-np.pi, np.pi))
