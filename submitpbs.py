...
class SubmitPBS():
  ...
  def __init__(self,
               ...
               walltime='1:00:00',
               set_env=True,
               scripts_dir='pbs_scripts',
               output_dir='pbs_output',
               wait_submitting=False,
               submit_label='',
               pbs_submit_cmd='qsub',
               limit_max_queued_jobs=0,
               source_dir=None,
               debug=False):
    ...
    if pbs_submission_infos is not None:
      ...
      wait_submitting = pbs_submission_infos.get('wait_submitting',
                                                 wait_submitting)
      ...
      pbs_submit_cmd = pbs_submission_infos.get('pbs_submit_cmd',
                                                pbs_submit_cmd)

      limit_max_queued_jobs = pbs_submission_infos.get('limit_max_queued_jobs',
                                                       limit_max_queued_jobs)

      # Use either the pre-created Unfilled script at the top, or a provided one (which could be loaded as a string from elsewhere)
      self.pbs_unfilled_script = pbs_submission_infos.get(
          'pbs_unfilled_script', PBS_SCRIPT)
    ...
    if self.debug:
      self.logger.info("SubmitPBS initialised:\n %s, %s, %s, %s" %
                       (self.working_directory, self.scripts_dir,
                        self.output_dir, self.pbs_options))
  ...
  def getEnvCommandString(self,
                          command="export",
                          env_keys_to_capture=frozenset(
  ...
  def update_running_jobs_number(self, max_name_length=16):
    ...
    if self.pbs_submit_cmd == 'qsub':
      # Using PBS
      queue_status = subprocess.Popen(
          ['qstat', '-u', username], stdout=subprocess.PIPE)
      jobname = self.submit_label[:max_name_length]
    elif self.pbs_submit_cmd == 'sbatch':
      # Using SLURM
      queue_status = subprocess.Popen(
          ['squeue', '-h', '-o', '%i %j %u %P'], stdout=subprocess.PIPE)
    ...
    if self.partition:
      self.num_queued_jobs = len([
          line for line in lines
          if username in line and jobname in line and self.partition in line
      ])
    else:
      self.num_queued_jobs = len(
          [line for line in lines if username in line and jobname in line])
  ...
  def wait_queue_not_full(self, sleeping_period=dict(min=30, max=180)):
    ...
    while self.num_queued_jobs >= self.limit_max_queued_jobs:
      ...
      sleep_time_rnd = np.random.randint(sleeping_period['min'],
                                         sleeping_period['max'])

      if self.debug:
        self.logger.info(
            "Queue full (%d queued/%d limit max), waiting %d sec..." %
            (self.num_queued_jobs, self.limit_max_queued_jobs, sleep_time_rnd))
  ...
  def add_to_submit_file(self, new_script_filename, command):
    ...
    with open(self.submit_fn, 'a') as submit_f:
      submit_f.write(self.pbs_submit_cmd + " " + new_script_filename + " #" +
                     command + '\n')

  ########################################

  def create_submit_job_parameters(self,
                                   pbs_command_infos,
  ...
  def create_simulation_command(self, pbs_command_infos):
    ...
    for (param, param_value) in pbs_command_infos['other_options'].items():
      if param_value is None:
        ...
        simulation_command += " --{param} {param_value}".format(
  ...
  def make_script(self, command):
    ...
    fn = os.path.join(self.scripts_dir,
                      "script." + str(hashlib.md5(command).hexdigest()))

    with open(fn, 'w') as f:
      # Construct the pbs options
      pbs_options = '\n'.join(
          ["#PBS -l " + k + "=" + v for (k, v) in self.pbs_options.items()])

      # Add SLURM options
      pbs_options += "\n#SBATCH -n1 --time={walltime} --mem-per-cpu={mem}".format(
          **self.pbs_options)
      ...
      filled_script = self.pbs_unfilled_script.format(
          pbs_options=pbs_options,
  ...
  def generate_submit_constrained_parameters_from_module_parameters(
    ...
    default_params = (('filtering_function',
                       None), ('filtering_function_parameters', None),
    ...
    if submission_parameters_dict['parameter_generation'] == 'grid':
      return self.generate_submit_constrained_parameters_grid(
          submission_parameters_dict['dict_parameters_range'],
          filtering_function=submission_parameters_dict['filtering_function'],
          filtering_function_parameters=submission_parameters_dict[
              'filtering_function_parameters'],
          pbs_submission_infos=submission_parameters_dict[
              'pbs_submission_infos'],
          submit_jobs=submission_parameters_dict['submit_jobs'])
    elif submission_parameters_dict['parameter_generation'] == 'random':
      return self.generate_submit_constrained_parameters_random(
          submission_parameters_dict['dict_parameters_range'],
          num_random_samples=submission_parameters_dict['num_random_samples'],
          filtering_function=submission_parameters_dict['filtering_function'],
          filtering_function_parameters=submission_parameters_dict[
              'filtering_function_parameters'],
          pbs_submission_infos=submission_parameters_dict[
  ...
  def generate_submit_constrained_parameters_random(
    ...
    while len(constrained_parameters) < num_random_samples:
      self.logger.info(
          "Parameters tested %d, found %d. %d requested. %.2f%%, %s left - %s"
          % (tested_parameters, len(constrained_parameters),
             num_random_samples, fill_parameters_progress.percentage(),
      ...
      if (filtering_function and filtering_function(
          ...
          filtering_function_parameters)) or (filtering_function is None):
        # Yes, all good
        ...
        if pbs_submission_infos:
          self.create_submit_job_parameters(
              pbs_submission_infos,
              force_parameters=new_parameters,
              submit=submit_jobs)
    ...
    for param_name, dict_specific_param_range in dict_parameters_range.items():
      if 'sampling_fct' not in dict_specific_param_range:
        ...
        if dict_specific_param_range['sampling_type'] == 'uniform':
          ...
          dict_specific_param_range['sampling_fct'] = functools.partial(
              lambda params: params['low'] + np.random.rand() * (params['high'] - params['low']),
              dict_specific_param_range)
        elif dict_specific_param_range['sampling_type'] == 'randint':
          dict_specific_param_range['sampling_fct'] = functools.partial(
              lambda params: np.random.randint(params['low'], params['high']),
              dict_specific_param_range)
        else:
          raise ValueError(
              'Sampling_type unknown and no sampling_fct provided... Cannot sample parameter %s'
              % param_name)
  ...
  def generate_submit_constrained_parameters_grid(
    ...
    cross_comb = utils.cross([[
        dict_parameters_range[param]['dtype'](param_elem)
        for param_elem in dict_parameters_range[param]['range'].tolist()
    ...
    for new_parameters in progress.ProgressDisplay(
      ...
      if (filtering_function is None) or (
          filtering_function
          and filtering_function(new_parameters, dict_parameters_range,
                                 filtering_function_parameters)):
  ...
  def generate_submit_sequential_optimisation(self,
                                              submission_parameters_dict):
    '''
    ...
    max_optimisation_iterations = submission_parameters_dict.get(
        'max_optimisation_iterations', 100)
    filtering_function = submission_parameters_dict.get(
        'filtering_function', None)
    filtering_function_parameters = submission_parameters_dict.get(
        'filtering_function_parameters', None)
    ...
    while parameters_tested < max_optimisation_iterations and not converged:
      self.logger.info(
          " >Parameters tested %d (max %d). %.2f%%, %s left - %s" %
          (parameters_tested, max_optimisation_iterations,
      ...
      # Check if the new parameters are within the constraints
      if (filtering_function is not None) and not filtering_function(
          new_parameters, dict_parameters_range,
          filtering_function_parameters):
      ...
      self.submit_minibatch_jobswrapper([new_parameters],
                                        submission_parameters_dict)
  ...
  def perform_cma_es_optimization(self, submission_parameters_dict):
    ...
    cma_population_size = submission_parameters_dict.get(
        'cma_population_size', None)
    cma_sigma0 = submission_parameters_dict.get('cma_sigma0', 1.0)
    cma_use_auto_scaling = submission_parameters_dict.get(
        'cma_use_auto_scaling', True)
    cma_iter_callback_function_infos = submission_parameters_dict.get(
        'cma_iter_callback_function_infos', None)
    cma_logger_do_plot = submission_parameters_dict.get(
        'cma_logger_do_plot', False)
    cma_logger_filename = submission_parameters_dict.get(
        'cma_logger_filename', 'logging_cmaes')
    cma_nan_replacement = submission_parameters_dict.get(
        'cma_nan_replacement', 1000000000.)
    cma_use_bounds = submission_parameters_dict.get('cma_use_bounds', False)
    cma_boundary_handling = submission_parameters_dict.get(
        'cma_boundary_handling', None)
    cma_tolx = submission_parameters_dict.get('cma_tolx', None)
    cma_tolfun = submission_parameters_dict.get('tolfun', None)
    cma_use_transforms = submission_parameters_dict.get(
        'cma_use_transforms', False)
      ...
      if 'x0' not in curr_param or 'scaling' not in curr_param:
        if 'low' in curr_param and 'high' in curr_param:
          ...
          if 'scaling' not in curr_param:
            curr_param['scaling'] = (
                curr_param['high'] - curr_param['low']) / (3. * cma_sigma0)
        else:
          raise ValueError(
              'No x0/scaling and high/low for variable %s, provide either one'
              % curr_param)

    # Extract the arrays of scalings for the parameters
    parameters_scalings = np.array([
        dict_parameters_range[curr_param_name]['scaling']
        for curr_param_name in parameter_names_sorted
    ])
    ...
    if cma_use_transforms:
      ...
      for param_name in parameter_names_sorted:
        if 'transform_fct' in dict_parameters_range[param_name]:
          all_transform_fct.append(
              dict_parameters_range[param_name]['transform_fct'])
        else:
          all_transform_fct.append(utils.identity)

        if 'transform_inv_fct' in dict_parameters_range[param_name]:
          all_transform_inv_fct.append(
              dict_parameters_range[param_name]['transform_inv_fct'])
        else:
    ...
    if cma_use_auto_scaling and not np.allclose(
        parameters_scalings, np.ones(len(parameter_names_sorted))):
      cma_options['CMA_stds'] = parameters_scalings
      self.logger.info("forcing scaling: %s" % parameters_scalings)
    if cma_use_bounds:
      cma_options['bounds'] = [[
          dict_parameters_range[param_key][bound_]
          for param_key in parameter_names_sorted
      ] for bound_ in ('low', 'high')]
    ...
    if cma_use_transforms:
      cma_options['transformation'] = [
          transform_params_fct, transform_inv_params_fct
      ]

    ### Instantiate CMA ES object.
    cma_es = cma.CMAEvolutionStrategy(
        self.cma_init_parameters(dict_parameters_range,
                                 parameter_names_sorted),
        cma_sigma0,
        inopts=cma_options)

    ## Instantiate a CMADataLogger. Will write to the current directory
    cma_log = cma.CMADataLogger(
        name_prefix=os.path.join(self.plot_output_dir,
                                 cma_logger_filename)).register(cma_es)
    ...
    try:
      while not cma_es.stop():
        ...
        parameters_candidates_dict = self.cma_list_parameters_array_to_dict(
            parameters_candidates_array, parameter_names_sorted,
            dict_parameters_range)

        # Make sure those parameters are acceptable
        wrong_parameters_indices = self.check_parameters_candidate(
            parameters_candidates_dict, parameter_names_sorted,
            dict_parameters_range)
        while len(wrong_parameters_indices) > 0:
          ...
          for wrong_parameters_i in wrong_parameters_indices:
            ...
            parameters_candidates_dict[
                wrong_parameters_i] = self.cma_parameters_array_to_dict(
                    parameters_candidates_array[wrong_parameters_i],
                    parameter_names_sorted, dict_parameters_range)
        ...
        if self.debug:
          self.logger.info("> Submitting minibatch (%d jobs)." %
                           len(parameters_candidates_array))
          for curr_param_dict_i, curr_param_dict in enumerate(
              parameters_candidates_dict):
            self.logger.info(" - {params}".format(
                params=utils.pprint_dict(
                    curr_param_dict, key_sorted=parameter_names_sorted)))

        ## Evaluate the parameters fitness, submit them!!
        fitness_results = self.submit_minibatch_jobswrapper(
            parameters_candidates_dict, submission_parameters_dict)

        # replace np.nan by large values...
        fitness_results[np.isnan(fitness_results)] = cma_nan_replacement

        if self.debug:
          self.logger.info(">> Minibatch completed (%d jobs)." %
                           len(parameters_candidates_array))
          for curr_param_dict_i, curr_param_dict in enumerate(
              parameters_candidates_dict):
            self.logger.info(" - {params} \t -> {result}".format(
                params=utils.pprint_dict(
                    curr_param_dict, key_sorted=parameter_names_sorted),
                result=fitness_results[curr_param_dict_i]))

        ## Do something after each CMA-ES iteration if desired
        if cma_iter_callback_function_infos is not None:
          cma_iter_callback_output = cma_iter_callback_function_infos[
              'function'](
                  locals(),
                  parameters=cma_iter_callback_function_infos['parameters'])
    ...
    self.logger.info(
        utils.pprint_dict(
            self.cma_parameters_array_to_dict(
                cma_es.best.x, parameter_names_sorted, dict_parameters_range),
            parameter_names_sorted))
    ...
    return dict(
        result_final=cma_es.result(),
        result_tracking_dict=self.result_tracking_dict,
        best_params=cma_es.best.get(),
        best_f=cma_es.best.f)

  def check_parameters_candidate(self, list_parameters_candidates_dict,
                                 parameter_names_sorted,
                                 dict_parameters_range):
    '''
    ...
    for parameters_candidate_i, parameters_candidate in enumerate(
        list_parameters_candidates_dict):

      for param_name in parameter_names_sorted:
        if (parameters_candidate[param_name] <
            dict_parameters_range[param_name]['low']) or (
                parameters_candidate[param_name] >
                dict_parameters_range[param_name]['high']):
          # wrong wrong wrong...
          wrong_parameters_indices.append(parameters_candidate_i)

    return wrong_parameters_indices

  def cma_parameters_dict_to_array(self, parameters_dict,
                                   parameter_names_sorted):
    '''
    ...
    return np.array(
        [parameters_dict[curr_param] for curr_param in parameter_names_sorted])

  def cma_list_parameters_dict_to_array(self, list_parameters_dict,
                                        parameter_names_sorted):
    '''
    ...
    return [
        self.cma_parameters_dict_to_array(parameters_dict,
                                          parameter_names_sorted)
        for parameters_dict in list_parameters_dict
    ]

  def cma_parameters_array_to_dict(
      self, parameters_array, parameter_names_sorted, dict_parameters_range):
    '''
            Takes an array of parameters and creates a dictionary.
        '''

    return dict(
        [(param_name, dict_parameters_range[param_name]['dtype'](
            parameters_array[param_name_i]))
         for param_name_i, param_name in enumerate(parameter_names_sorted)])

  def cma_list_parameters_array_to_dict(self, list_parameters_array,
                                        parameter_names_sorted,
                                        dict_parameters_range):
    '''
    ...
    return [
        self.cma_parameters_array_to_dict(
            parameters_array, parameter_names_sorted, dict_parameters_range)
        for parameters_array in list_parameters_array
    ]
  ...
  def cma_init_parameters(self, dict_parameters_range, parameter_names_sorted):
    ...
    return np.array([
        dict_parameters_range[curr_param]['dtype'](
            dict_parameters_range[curr_param]['x0'])
        for curr_param in parameter_names_sorted
    ])

  ##########################################################################

  def submit_minibatch_jobswrapper(self, parameters_to_submit,
                                   submission_parameters_dict):
    '''
    ...
    pbs_submission_infos = submission_parameters_dict.get(
        'pbs_submission_infos', None)
    sleeping_period = submission_parameters_dict.get('sleeping_period',
                                                     dict(min=60, max=180))
    submit_jobs = submission_parameters_dict.get('submit_jobs', False)
    result_callback_function_infos = submission_parameters_dict.get(
        'result_callback_function_infos', None)
    wait_jobs_completed = submission_parameters_dict.get(
        'wait_jobs_completed', True)

    # Track status of parameters: parameters -> dict(status=['waiting', 'submitted', 'completed'], result=None, jobwrapper=None, parameters=None)

    completed_parameters_progress = progress.Progress(
        len(parameters_to_submit))
    # parameters_tested = 0
    job_names_ordered = []
    ...
    for current_parameters in parameters_to_submit:
      ...
      job_submission_parameters = self.prepare_job_parameters(
          current_parameters, pbs_submission_infos)

      # Create job
      new_job = jobwrapper.JobWrapper(
          job_submission_parameters,
          session_id=job_submission_parameters['session_id'],
          debug=False)
    ...
    if self.debug:
      self.logger.info(
          "-> submitted minibatch, %d jobs. %s computation" %
          (len(parameters_to_submit),
           job_submission_parameters.get('result_computation', 'no')))

    if wait_jobs_completed:
      ## Wait for Jobs to be completed (could do another version where you send multiple jobs before waiting)
      self.wait_all_jobs_collect_results(
          result_callback_function_infos=result_callback_function_infos,
          sleeping_period=sleeping_period,
          completion_progress=completed_parameters_progress,
          pbs_submission_infos=pbs_submission_infos)
  ...
  def prepare_job_parameters(self, new_parameters, pbs_submission_infos):
    ...
    job_submission_parameters['job_action'] = job_submission_parameters[
        'action_to_do']
  ...
  def track_new_job(self, job, parameters=dict()):
    ...
    self.jobs_tracking_dict[job.job_name] = dict(
        status='waiting',
        job=job,
        job_submission_parameters=job.experiment_parameters,
        result=None,
        parameters=parameters,
        time_started=None,
        number_submissions=0)
    self.result_tracking_dict[tuple(parameters.items())] = dict(
        jobname=job.job_name, result=None)
  ...
  def complete_job(self, job_name):
    ...
    self.jobs_tracking_dict[job_name]['result'] = self.jobs_tracking_dict[
        job_name]['job'].get_result()
    self.result_tracking_dict[tuple(
        self.jobs_tracking_dict[job_name]['parameters'].items())][
            'result'] = self.jobs_tracking_dict[job_name]['result']

  def submit_jobwrapper(self,
                        job,
                        pbs_submission_infos,
                        submit=False,
    ...
    if not job.check_completed():
      ...
      self.create_submit_job_parameters(
          pbs_submission_infos_bis, submit=submit)
  ...
  def wait_all_jobs_collect_results(self,
                                    result_callback_function_infos=None,
                                    sleeping_period=dict(min=60, max=180),
                                    completion_progress=None,
                                    pbs_submission_infos=None,
                                    max_number_submissions=1):
    '''
    ...
    submitted_job_names = collections.deque([
        job_name for job_name, job_dict in self.jobs_tracking_dict.iteritems()
      ...
      if self.jobs_tracking_dict[current_job_name]['status'] == 'completed':
        ...
        if self.jobs_tracking_dict[current_job_name]['job'].check_completed():
          ...
          if result_callback_function_infos is not None:
            result_callback_function_infos['function'](
                job=self.jobs_tracking_dict[current_job_name]['job'],
                parameters=result_callback_function_infos['parameters'])

          if self.debug:
            str_completion = "Job {0} done. Result: {1}. {2} left.".format(
                current_job_name,
                self.jobs_tracking_dict[current_job_name]['result'],
                len(submitted_job_names))

            # (this ridiculous len(str) business is to have some pretty output, as we write \r)
            self.logger.info(str_completion + " " *
                             (last_str_len - len(str_completion)))
          ...
          ) - self.jobs_tracking_dict[current_job_name]['time_started']
          if (waited_time > utils.convert_deltatime_str_to_seconds(
              self.pbs_options['walltime']) * 1.2):
            # Waited too long...
            # try to resubmit it!
            if self.debug:
              self.logger.info(
                  "Waited more than walltime for job %s, resubmitting it (%d/%d)         "
                  % (current_job_name, self.jobs_tracking_dict[
                      current_job_name]['number_submissions'],
                     max_number_submissions))
            ...
            if self.jobs_tracking_dict[current_job_name]['number_submissions'] < max_number_submissions:
              ...
              self.submit_jobwrapper(
                  self.jobs_tracking_dict[current_job_name]['job'],
                  pbs_submission_infos,
                  submit=True)
            else:
          ...
          if self.debug:
            status_str = "Waiting on Job %s, waited %d/%d. Sleeping for %d sec now." % (
                current_job_name, waited_time,
                utils.convert_deltatime_str_to_seconds(self.pbs_options[
                    'walltime']) * 1.2, sleep_time_rnd)

            if completion_progress is not None:
              # Also add how much time to completion
              status_str += " %.2f%%, %s left - %s. %d jobs left         " % (
                  completion_progress.percentage(),
                  completion_progress.time_remaining_str(),
                  completion_progress.eta_str(), len(submitted_job_names) + 1)
...
def test_sequential_optimisation():
  ...
  submission_parameters_dict = dict(
      ...
      dict_parameters_range=dict(
          T=dict(sampling_type='randint', low=1, high=6, dtype=int),
          sigmax=dict(
      ...
      pbs_submission_infos=dict(
          description='Testing sequential optim',
          ...
          walltime='1:00:00',
          memory='2gb',
          simul_out_dir=os.path.join(os.getcwd(),
                                     run_label.format(**locals())),
  ...
  submission_parameters_dict['result_callback_function_infos'] = dict(
      function=best_parameters_callback,
      parameters=dict(best_parameters_seen=best_parameters_seen))

  # Create a SubmitPBS
  submit_pbs = SubmitPBS(
      pbs_submission_infos=submission_parameters_dict['pbs_submission_infos'],
      debug=True)

  # Run the big useless sequential optimisation
  submit_pbs.generate_submit_sequential_optimisation(
      submission_parameters_dict)
...
def test_cmaes_optimisation():
  ...
  dict_par_range = dict(
      M=dict(low=5, high=800, dtype=int),
  ...
  submission_parameters_dict = dict(
      ...
      pbs_submission_infos=dict(
          description='Testing CMA-ES optim',
          command='python $WORKDIR/experimentlauncher.py',
          other_options=dict(
  ...
  def best_parameters_callback(job, parameters=None):
    best_parameters_seen = parameters['best_parameters_seen']
    if best_parameters_seen['result'] is None or job.get_result(
    ) <= best_parameters_seen['result'] and not np.isnan(job.get_result()):
      # New best parameter!
      best_parameters_seen['result'] = job.get_result()
      best_parameters_seen['job_name'] = job.job_name
      best_parameters_seen['parameters'] = job.experiment_parameters
  ...
  cma_iter_parameters = dict(
      ax=None, candidates=[], fitness=[], dataio=data_io)
  ...
  def cma_iter_plot_contourf_candidates(all_variables, parameters=None):
    ...
    # Do a plot
    parameters['ax'] = utils.contourf_interpolate_data(
        np.array(candidates),
        np.array(fitness),
        xlabel='Ratio_conj',
        ylabel='sigma x',
    ...
    if parameters['dataio'] is not None:
      parameters['dataio'].save_variables_default(locals(),
                                                  ['candidates', 'fitness'])
      parameters['dataio'].save_current_figure(
          'cmaes_optim_landscape_{label}_{unique_id}.pdf')

  submission_parameters_dict['cma_iter_callback_function_infos'] = dict(
      function=cma_iter_plot_contourf_candidates,
      parameters=cma_iter_parameters)
...
def test_cmaes_optimisation_3d():
  ...
  dict_par_range = dict(
      M=dict(low=10, high=500, dtype=int),
      ratio_conj=dict(low=0.0, high=1.0, dtype=float),
      sigmax=dict(low=0.0001, high=2.0, dtype=float))
  ...
  def cma_iter_plot_contourf_candidates(all_variables, parameters=None):
    ...
    if parameters['dataio'] is not None:
      parameters['dataio'].save_variables_default(locals(),
                                                  ['candidates', 'fitness'])
      parameters['dataio'].save_current_figure(
          'cmaes_optim_landscape_scatter3d_{label}_{unique_id}.pdf')
...
if __name__ == '__main__':
  ...
  if False:
    cwd = os.getcwd()
    submit_pbs = SubmitPBS(
        working_directory=os.path.join(cwd, 'simple_pbs_test'), debug=True)
  ...
  if False:
    ...
    pbs_submission_infos = dict(
        description=
        'Getting the full memory curves for all the parameters compatible with the experimental Fisher information',
        command='echo "Testing PBS"',
        other_options=dict(option1='1', option2=2),
        walltime='10:00:00',
        memory='2gb',
        simul_out_dir=os.path.join(cwd, 'testing_pbs'))
    dict_parameters_range = dict(
        param1=dict(sampling_type='uniform', low=0.01, high=15.0, dtype=float),
        param2=dict(sampling_type='uniform', low=0.01, high=0.8, dtype=float))
    filtering_function = lambda parameters, dict_parameters_range, other_options: parameters['param1'] > 1.0

    submit_pbs = SubmitPBS(
        pbs_submission_infos=pbs_submission_infos, debug=True)
