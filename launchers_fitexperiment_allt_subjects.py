  ...
  for repet_i in xrange(all_parameters['num_repetitions']):
    ...
    def compute_everything(self, parameters):
      ...
      if not parameters['inference_method'] == 'none':
        ...
        results['result_emfit_mse_scaled'] = emfits_distances['mse_scaled']

        # Compute fisher info
        print " compute fisher info"
        results['result_fi_theo'] = (self.sampler.estimate_fisher_info_theocov(
            use_theoretical_cov=False))
        results['result_fi_theocov'] = (
            self.sampler.estimate_fisher_info_theocov(use_theoretical_cov=True)
        )
