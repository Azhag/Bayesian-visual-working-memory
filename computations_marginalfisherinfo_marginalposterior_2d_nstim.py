...
def main(to_plot=[]):
    ...
    if 1 in to_plot:
        ...
        deriv_mu[0] = -kappa * np.sin(
            pref_angles[:, 0] - item1_theta1) * population_code_response_2D(
                item1_theta1,
                item1_theta2,
                pref_angles,
                N=N,
                kappa=kappa,
                amplitude=amplitude)
        deriv_mu[1] = -kappa * np.sin(
            pref_angles[:, 1] - item1_theta2) * population_code_response_2D(
                item1_theta1,
                item1_theta2,
                pref_angles,
                N=N,
        ...
        if n_items > 1:
            print "FI 2-4 obj"
            search_progress = progress.Progress(all_angles_1D.size
                                                **(2 * (n_items - 1)))

            print search_progress.total_work

            FI_2d_2obj_search = np.zeros((all_angles_1D.size,
                                          all_angles_1D.size, 4, 4))
            inv_FI_2d_2obj_search = np.zeros((all_angles_1D.size,
                                              all_angles_1D.size, 4, 4))
            ...
            if n_items > 2:
                ...
                if use_pytables:
                    f_table = tb.openFile(
                        'inv_FI_3obj_search.hdf',
                        'w',
                        title="Inv Fisher Info 2d 3 objects")
                    atom = tb.Atom.from_dtype(inv_FI_2d_2obj_search.dtype)
                    compression_filter = tb.Filters(
                        complib='blosc', complevel=5)
                    # compression_filter = None
                    inv_FI_2d_3obj_search = f_table.createCArray(
                        f_table.root,
                        'FI',
                        atom, (all_angles_1D.size**4, 6, 6),
                        filters=compression_filter)

                    print inv_FI_2d_3obj_search
                else:
                    inv_FI_2d_3obj_search = np.zeros((all_angles_1D.size**4, 6,
                                                      6))
            ...
            for i, item2_theta1 in enumerate(all_angles_1D):
                for j, item2_theta2 in enumerate(all_angles_1D):
                    if (enforce_distance(
                            item1_theta1, item2_theta1,
                            min_distance=min_distance) and enforce_distance(
                                item1_theta2,
                                item2_theta2,
                        ...
                        FI_2d_2obj_search[i, j] = np.dot(
                            deriv_mu, deriv_mu.T) / (2. * sigma**2.)

                        inv_FI_2d_2obj_search[i, j] = np.linalg.inv(
                            FI_2d_2obj_search[i, j])
                            ...
                            for k in xrange(all_angles_1D.size):
                                ...
                                for l in xrange(all_angles_1D.size):
                                    ...
                                    if (enforce_distance(
                                        ...
                                        deriv_mu[4] = -kappa * np.sin(
                                            pref_angles[:, 0] - item3_theta1
                                        ) * population_code_response_2D(
                                            item3_theta1,
                                            item3_theta2,
                                            pref_angles,
                                            ...
                                            kappa=kappa,
                                            amplitude=amplitude)
                                        deriv_mu[5] = -kappa * np.sin(
                                            pref_angles[:, 1] - item3_theta2
                                        ) * population_code_response_2D(
                                            item3_theta1,
                                            item3_theta2,
                                            pref_angles,
                                        ...
                                        FI_2d_3obj_search_curr = np.dot(
                                            deriv_mu, deriv_mu.T) / (
                                                3. * sigma**2.)

                                        # print "bla"
                                        inv_FI_2d_3obj_search[
                                            i * all_angles_1D.size *
                                            all_angles_1D.size * all_angles_1D.
                                            size + j * all_angles_1D.
                                            size * all_angles_1D.size +
                                            k * all_angles_1D.size +
                                            l, :, :] = np.linalg.inv(
                                                FI_2d_3obj_search_curr)
        ...
        if n_items > 1:
            FI_2d_2obj_marginal = np.mean(
                np.mean(np.ma.masked_equal(FI_2d_2obj_search, 0.0), axis=0),
                axis=0)
            inv_FI_2d_2obj_marginal_bis = np.mean(
                np.mean(
                    np.ma.masked_equal(inv_FI_2d_2obj_search, 0.0), axis=0),
                axis=0)
            inv_FI_2d_2obj_marginal = np.linalg.inv(FI_2d_2obj_marginal)

            # Some stats
            inv_FI_nbitems_effects = [
                inv_FI_2d_1obj_search[0, 0], inv_FI_2d_2obj_marginal[0, 0],
            ...
            N_effect_withlocal = [
                inv_FI_2d_1obj_search[0, 0], inv_FI_2d_2obj_marginal_bis[0, 0]
            ]
            ...
            if n_items > 2:
                ...
                inv_FI_2d_3obj_marginal_bis = np.mean(
                    np.ma.masked_equal(inv_FI_2d_3obj_search, 0.0), axis=0)

                # inv_FI_nbitems_effects.extend([inv_FI_2d_3obj_marginal[0, 0], inv_FI_2d_3obj_marginal_bis[0, 0]])
                inv_FI_nbitems_effects.extend(
                    [inv_FI_2d_3obj_marginal_bis[0, 0]])
            ...
            pcolor_2d_data(
                inv_FI_2d_2obj_search[:, :, 0, 0],
                x=all_angles_1D,
                y=all_angles_1D,
                ticks_interpolate=0)

            dataio.save_current_figure(
                'inv_FI_2d_2obj_search_{label}_{unique_id}.pdf')

            plt.figure()
            plt.bar(
                np.arange(len(inv_FI_nbitems_effects)),
                inv_FI_nbitems_effects,
                width=0.5)
            plt.xticks(
                np.arange(len(inv_FI_nbitems_effects)) + 0.25, [
                    '1 obj', '2 obj invmean', '2 obj meaninv', '3 obj invmean',
                    '3 obj meaninv'
                ])

            dataio.save_current_figure(
                'bars_if_1_2_3_objects_{label}_{unique_id}.pdf')
            ...
            dataio.save_variables([
                'N_effect_withlocal', 'N_effect_nolocal',
                'inv_FI_2d_2obj_search', 'inv_FI_nbitems_effects',
                'FI_2d_2obj_marginal', 'inv_FI_2d_2obj_marginal',
                'inv_FI_2d_2obj_marginal_bis', 'FI_2d_3obj_marginal',
                'inv_FI_2d_3obj_marginal', 'inv_FI_2d_3obj_marginal_bis',
                'FI_2d_1obj_search', 'inv_FI_2d_1obj_search'
            ], locals())
    ...
    if 2 in to_plot:
        ...
        def inv_FI_nobj_fixoneitem(items_thetas):
            K = len(items_thetas)
            deriv_mu = np.zeros((2 * (K + 1), N))
            deriv_mu[0] = -kappa * np.sin(
                pref_angles[:, 0]
            ) * population_code_response_2D(
                0.0, 0.0, pref_angles, N=N, kappa=kappa, amplitude=amplitude)
            deriv_mu[1] = -kappa * np.sin(
                pref_angles[:, 1]
            ) * population_code_response_2D(
                0.0, 0.0, pref_angles, N=N, kappa=kappa, amplitude=amplitude)
            for i in xrange(K):
                deriv_mu[2 * i + 2] = -kappa * np.sin(
                    pref_angles[:, 0] - items_thetas[i][0]
                ) * population_code_response_2D(
                    items_thetas[i][0],
                    items_thetas[i][1],
                    pref_angles,
                    ...
                    kappa=kappa,
                    amplitude=amplitude)
                deriv_mu[2 * i + 3] = -kappa * np.sin(
                    pref_angles[:, 1] - items_thetas[i][1]
                ) * population_code_response_2D(
                    items_thetas[i][0],
                    items_thetas[i][1],
                    pref_angles,
        ...
        def enforce_distance_set(new_item, other_items, min_distance=0.001):
            return all(
                enforce_distance(
                    new_item[0], other_item[0], min_distance=min_distance)
                and enforce_distance(
                    new_item[1], other_item[1], min_distance=min_distance)
        ...
        def inv_FI_one_sample(n_items=4, return_items=False):
            ...
            for item_i in xrange(n_items):
                inv_FI_allobj[item_i], FI_allobj[
                    item_i] = inv_FI_nobj_fixoneitem(all_items[1:])

                # Add an extra item
                new_item = -np.pi + 2 * np.pi * np.random.random(2)
                while not enforce_distance_set(new_item, all_items,
                                               min_distance):
                    new_item = -np.pi + 2 * np.pi * np.random.random(2)
        ...
        for i in xrange(n_samples):
            ...
            if i % 1000 == 0:
                sys.stdout.write("%.1f%%, %s \n %d %s %s %s\n" %
                                 (search_progress.percentage(),
                                  search_progress.time_remaining_str(), i,
                                  inv_FI_allobj_cum * n_samples / (i + 1),
                                  np.sqrt(inv_FI_all_obj_var_cum * n_samples /
                                          (i + 1)), power_law_fits))
                sys.stdout.flush()

            # Get sample of invFI and FI
            if plot_item2_effect:
                sample_invFI, sample_FI, new_pos_item2 = inv_FI_one_sample(
                    n_items=n_items, return_items=True)
            ...
            if i > min_num_samples_std:
                inv_FI_all_obj_var_cum += (
                    sample_invFI - inv_FI_allobj_cum * n_samples /
                    (i + 1.))**2. / float(n_samples)
                FI_all_obj_var_cum += (
                    1. / sample_FI - FI_all_obj_cum * n_samples /
                    (i + 1.))**2. / float(n_samples)

            # Saves some stuff
            if i % save_every == 0:
                # Estimate power law fit
                power_law_fits = fit_powerlaw(
                    np.arange(1, n_items + 1),
                    inv_FI_allobj_cum * n_samples / (i + 1))

                dataio.save_variables([
                    'inv_FI_allobj_cum', 'inv_FI_all_obj_var_cum',
                    'power_law_fits', 'n_samples', 'i', 'min_distance',
                    'n_items'
                ], locals())
                ...
                if i > 10 and plot_item2_effect:
                    ...
                    contourf_interpolate_data(
                        np.array(item2_positions),
                        np.array(all_inv_FI_2obj),
                        xlabel='$\phi_2$',
                        ylabel='$\psi_2$',
                        title=
                        'Inverse Fisher Information for $\phi_1 = \psi_1 = 0$, varying $\phi_2, \psi_2$',
                        fignum=3,
                        show_colorbar=False,
                        show_scatter=False,
                    ...
                    dataio.save_current_figure(
                        'contourf_IF_2objeffect_{label}_{unique_id}.png')
