...
stddev_to_kappa = np.vectorize(
    stddev_to_kappa_single,
    doc='''
        Converts stddev to kappa
...
def test_stability_stddevtokappa(target_kappa=2.):
  ...
  for i in xrange(nb_iterations):
    if i == 0:
      ...
      kappa_evolution[i] = stddev_to_kappa(
          kappa_to_stddev(kappa_evolution[i - 1]))
...
def compute_mean_std_circular_data(angles):
  ...
  # Population mean
  angle_mean_error = angle_population_mean(
      angle_population_vec=angle_mean_vector)

  # Circular standard deviation estimate
  angle_std_dev_error = angle_circular_std_dev(
      angle_population_vec=angle_mean_vector)
  ...
  return dict(
      mean=angle_mean_error,
      std=angle_std_dev_error,
      population_vector=angle_mean_vector,
      bias=angle_bias)


def compute_precision_samples(samples,
                              square_precision=True,
  ...
  precision = compute_angle_precision_from_std(
      angle_circular_std_dev(samples), square_precision=square_precision)
...
def enforce_distance_set(new_item, other_items, min_distance=0.001):
  return all(
      enforce_distance(new_item[0], other_item[0], min_distance=min_distance)
