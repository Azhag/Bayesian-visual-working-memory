...
if __name__ == '__main__':
    ...
    if True:
        ...
        FI_2d_2obj_marginal = np.mean(np.mean(np.ma.masked_equal(np.squeeze(FI_2d_search), 0.0), axis=0), axis=0)
        inv_FI_2d_2obj_marginal_bis = np.mean(np.mean(np.ma.masked_equal(np.squeeze(inv_FI_2d_search), 0.0), axis=0), axis=0)
        ...
        plt.figure()
        plt.bar([0, 1, 2], [inv_FI_2d_1obj_search[0, 0, 0, 0], inv_FI_2d_2obj_marginal[0, 0], inv_FI_2d_2obj_marginal_bis[0, 0]], width=0.5)
