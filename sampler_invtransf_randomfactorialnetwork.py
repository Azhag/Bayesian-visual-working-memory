...
def loglike_theta_fct_single(new_theta, (
    thetas, datapoint, rn, theta_mu, theta_kappa, ATtcB, sampled_feature_index,
    mean_fixed_contrib, inv_covariance_fixed_contrib)):
  ...
  return -0.5 * np.dot(like_mean,
                       np.dot(inv_covariance_fixed_contrib, like_mean))
  # return -1./(2*0.2**2)*np.sum(like_mean**2.)


def loglike_theta_fct_single_min(
    x, thetas, datapoint, rn, theta_mu, theta_kappa, ATtcB,
    sampled_feature_index, mean_fixed_contrib, inv_covariance_fixed_contrib):
  return -loglike_theta_fct_single(x, (
      thetas, datapoint, rn, theta_mu, theta_kappa, ATtcB,
      sampled_feature_index, mean_fixed_contrib, inv_covariance_fixed_contrib))


def like_theta_fct_single(x, thetas, datapoint, rn, theta_mu, theta_kappa,
                          ATtcB, sampled_feature_index, mean_fixed_contrib,
                          inv_covariance_fixed_contrib):
  return np.exp(
      loglike_theta_fct_single(
          x, (thetas, datapoint, rn, theta_mu, theta_kappa, ATtcB,
              sampled_feature_index, mean_fixed_contrib,
              inv_covariance_fixed_contrib)))
...
class Sampler:
  ...
  Continuous angles Theta, with Von Mise prior. x | Theta ~ Normal.
  Using the population codes directly y_t | x_t, y_{t-1} ~ Normal
  """

  def __init__(self,
               data_gen,
               tc=None,
               theta_prior_dict=dict(kappa=0.01, gamma=0.0),
               n_parameters=dict(),
               sigma_output=0.0,
               parameters_dict=None,
               renormalize_sigma_output=False,
               lapse_rate=0.0,
               inv_cdf_bins=1000):
  ...
  def init_sampling_parameters(self, parameters_dict=None):
    ...
    default_parameters = dict(
        ...
        slice_width=np.pi / 16.,
        slice_jump_prob=0.3,
        integrate_tc_out=False,
        num_sampling_passes=1,
        cued_feature_type='single')
  ...
  def init_theta(self):
    ...
    self.theta = np.random.vonmises(
        self.theta_gamma, self.theta_kappa, size=(self.N, self.R))
  ...
  def init_theta_single_cued(self):
    ...
    print "-> init theta, feature %d cued, time %d" % (
        self.data_gen.cued_features[0, 0],
        self.data_gen.cued_features[0, 1] + 1)

    # Assign the cued ones now
    #   stimuli_correct: N x T x R
    #   cued_features:   N x (recall_feature, recall_time)
    self.theta[np.arange(self.N), self.data_gen.
               cued_features[:self.N, 0]] = self.data_gen.stimuli_correct[
    ...
    self.theta_to_sample = np.array(
        [[r for r in xrange(self.R) if r != self.data_gen.cued_features[n, 0]]
         for n in xrange(self.N)],
        dtype='int')

    assert (
        self.theta_to_sample.shape[1] == 1), "Not sure this will work anymore"
    self.sampled_feature_index = 0
  ...
  def init_theta_all_cued(self):
    ...
    self.theta[:, 1:] = self.data_gen.stimuli_correct[np.arange(
        self.N), self.data_gen.cued_features[:self.N, 1], 1:]

    # Construct the list of uncued features, which should be sampled
    self.theta_to_sample = self.theta_target_index[:self.N]

  def init_cache_parameters(self):
    """Most of our multiplicative factors are fixed, so precompute them, for
    all tc.
    ...
    for t in xrange(self.T):
      (self.ATtcB[t], self.mean_fixed_contrib[t], self.noise_covariance,
       self.inv_covariance_fixed_contrib) = self.precompute_parameters(t)

    # Compute the normalization
    self.compute_normalization()

    # Setup the inverse transform parameters
    self.angle_space = np.linspace(
        -np.pi, np.pi, self.inv_cdf_bins, endpoint=False)
    self.inverse_cdfs = dict()
    # self.compute_inverse_transform_parameters()

  def init_output_noise(self, sigma_output, renormalize=True):
    """The output noise is added after samples from the posterior are taken.
    Adds another level of randomness. Should count it in the utils.BIC.

    Given a level of output noise (sigma_output), stores the
    corresponding kappa_output.
    """
  ...
  def precompute_parameters(self, t):
    ...
    mean_fixed_contrib = self.n_means_end[t] + np.dot(ATmtc,
                                                      self.n_means_start[t])
    ...
    covariance_fixed_contrib = ATmtc**2. * (
        self.sigma_x**2. + self.sigma_y**2.) * np.eye(
            self.M) + self.sigma_baseline**2. * np.eye(self.M)
    if self.T > 1:
  ...
  def compute_normalization(self):
    ...
    for n in xrange(self.N):
      ...
      params = (self.theta[n], self.NT[n], self.random_network,
                self.theta_gamma, self.theta_kappa, self.ATtcB[self.tc[n]],
                self.sampled_feature_index,
                self.mean_fixed_contrib[self.tc[n]],
  ...
  def compute_inverse_transform_parameters(self, n):
    ...
    for x_i, x in enumerate(self.angle_space):
      curr_likelihood[x_i] = like_theta_fct_single(
          x, self.theta[n], self.NT[n], self.random_network, self.theta_gamma,
          self.theta_kappa, self.ATtcB[self.tc[n]], self.sampled_feature_index,
          self.mean_fixed_contrib[self.tc[n]],
          self.inv_covariance_fixed_contrib)
    ...
    self.inverse_cdfs[n] = spinter.interp1d(
        curr_cumulative, self.angle_space, kind='linear', assume_sorted=True)
  ...
  def sample_theta(self, return_samples=False, subset_theta=None, debug=False):
    """Sample the thetas Need to use a slice sampler, as we do not know the
    ...
    if debug:
      if self.selection_method == 'last':
        print "Sampling theta: %d samples, %d burnin, select last" % (
            self.num_samples, self.burn_samples)
      else:
        print "Sampling theta: %d samples, %d selection, %d burnin" % (
            self.num_samples, self.selection_num_samples, self.burn_samples)
    ...
    if debug:
      search_progress = progress.Progress(
          (self.R - 1) * permuted_datapoints.size)
    ...
    for n in permuted_datapoints:
      ...
      for sampled_feature_index_i, sampled_feature_index in enumerate(
          permuted_features):
        # Handle lapse rate
        has_lapsed, sampled_orientation = self.handle_lapse_sample(
            cache_randomdraws[n, sampled_feature_index_i])

        if not has_lapsed:
          # Get samples from the current memory distribution
          if not self.integrate_tc_out:
            samples = self.get_samples_theta_current_tc(
                n, sampled_feature_index=sampled_feature_index)
          else:
            samples = self.get_samples_theta_tc_integratedout(
                n, sampled_feature_index=sampled_feature_index)
          ...
          if self.selection_method == 'median':
            sampled_orientation = np.median(
                samples[-self.selection_num_samples:], overwrite_input=True)
        ...
        self.theta[n, sampled_feature_index] = utils.wrap_angles(
            sampled_orientation)
        ...
        if debug:
          ...
          line = "%.2f%%, %s - %s" % (search_progress.percentage(),
  ...
  def handle_lapse_sample(self, random_draw=None):
    """Handle Lapse rate, where a certain proportion of sampling runs are
    simply dropped and randomly set to U[-pi, pi]

    Depends on self.lapse_rate. Samples one U[0, 1], could be
    precomputed...
    """
    should_lapse = False
  ...
  def get_samples_theta_tc_integratedout(self, n, sampled_feature_index=0):
    """Sample theta, with tc integrated out. Use rejection sampling (or
    something), discarding some samples.

    Note: the actual number of samples returned is random, but around num_samples.
    """

    raise NotImplementedError(
        "!!!! OLD/DEPRECATED INTEGRATION... CHECK ME !!!")
  ...
  def add_output_noise(self, sample):
    """Assume that samples are corrupted by some extra Von Mises noise,
    centered at the current sample and with a kappa set by self.sigma_output.
  ...
  def set_theta(self, new_thetas):
    """Update thetas to a given value (most likely to experimentally measured
    ones)"""
  ...
  def set_theta_max_likelihood(self, num_points=100, post_optimise=True):
    ...
    for n in progress.ProgressDisplay(
        np.arange(self.N), display=progress.SINGLE_LINE):
      ...
      if post_optimise:
        self.theta[n, self.sampled_feature_index] = spopt.fmin(
            loglike_theta_fct_single_min,
            all_angles[np.argmax(llh)],
            args=params,
            disp=False)[0]
      else:
    ...
    # Add output noise if desired.
    self.theta[:,
               self.sampled_feature_index] = self.add_output_noise_vectorized(
                   self.theta[:, self.sampled_feature_index])
  ...
  def collect_responses(self):
    ...
    nontargets = self.data_gen.stimuli_correct[
        ...
        self.theta_target_index[:self.N]].T
  ...
  def compute_bic(self, K=None, integrate_tc_out=False, LL=None,
    ...
    if LL is None:
      LL = self.compute_loglikelihood(
          integrate_tc_out=integrate_tc_out, precision=precision)
  ...
  def compute_loglikelihood(self, integrate_tc_out=False, precision=200):
    """Compute the summed loglikelihood for the current setting of thetas and
    using the likelihood defined in loglike_theta_fct_single.

    - integrate_tc_out:  use the current tc, or should integrate over possible recall times?
    """
    return np.nansum(
        self.compute_loglikelihood_N(
            integrate_tc_out=integrate_tc_out, precision=precision))

  def compute_loglikelihood_N(self,
                              integrate_tc_out=False,
                              precision=200,
                              lapse_likelihood_lower_bound=False):
    """Compute the loglikelihood for each datapoint, using the current setting
    of thetas and likelihood functions. Uses the normalisation.

    - integrate_tc_out: use current tc, or should integrate over recall times?
    """
    LL = 0
    if self.sigma_output > 0.0:
      LL += self.compute_loglikelihood_N_convolved_output_noise(
          precision=precision)
    else:
      if integrate_tc_out:
    ...
    if self.lapse_rate > 0.0:
      ...
      if lapse_likelihood_lower_bound:
        ...
        LL = np.log(self.lapse_rate) - np.log(2. * np.pi) + np.log1p(
            (1. - self.lapse_rate) * 2. * np.pi /
            (self.lapse_rate) * np.exp(LL))

    return LL

  def compute_loglikelihood_top90percent(self,
                                         integrate_tc_out=False,
                                         precision=200,
                                         all_loglikelihoods=None):
    """Backward compatibility function when p was fixed to 90%"""
    return self.compute_loglikelihood_top_p_percent(
        0.9,
        integrate_tc_out=integrate_tc_out,
        precision=precision,
        all_loglikelihoods=all_loglikelihoods)

  def compute_loglikelihood_top_p_percent(self,
                                          p,
                                          integrate_tc_out=False,
                                          precision=200,
                                          all_loglikelihoods=None):
    """Compute the loglikelihood for each datapoint, just like
    compute_loglikelihood and compute_loglikelihood_N, but now only sums the
    top p% results."""

    if all_loglikelihoods is None:
      all_loglikelihoods = self.compute_loglikelihood_N(
          integrate_tc_out=integrate_tc_out, precision=precision)

    return np.nansum(
        np.sort(all_loglikelihoods)[int(np.round(self.N * (1 - p))):])

  def compute_loglikelihood_current_tc(self):
    """Compute the loglikelihood for the current setting of thetas and tc and
    using the likelihood defined in loglike_theta_fct_single."""
    ...
    for n in xrange(self.N):
      # Pack the parameters for the likelihood function
      params = (self.theta[n].copy(), self.NT[n], self.random_network,
                self.theta_gamma, self.theta_kappa, self.ATtcB[self.tc[n]],
                self.sampled_feature_index,
                self.mean_fixed_contrib[self.tc[n]],
                self.inv_covariance_fixed_contrib)
      ...
      loglikelihood[n] = loglike_theta_fct_single(
          self.theta[n, self.sampled_feature_index], params)
  ...
  def compute_loglikelihood_tc_integratedout(self):
    """Compute the loglikelihood for the current setting of thetas and using
    the likelihood defined in loglike_theta_fct_single Integrates tc out."""
    ...
    for n in xrange(self.N):
      for tc in xrange(self.T):
        # Pack the parameters for the likelihood function
        params = (self.theta[n].copy(), self.NT[n], self.random_network,
                  self.theta_gamma, self.theta_kappa, self.ATtcB[tc],
                  self.sampled_feature_index, self.mean_fixed_contrib[tc],
                  self.inv_covariance_fixed_contrib)

        # Compute the loglikelihood for the current datapoint
        loglikelihood[n, tc] = loglike_theta_fct_single(
            self.theta[n, self.sampled_feature_index], params)
  ...
  def compute_loglikelihood_N_convolved_output_noise(self, precision=100):
    ...
    for n in xrange(self.N):
      ...
      convolved_posterior = self.compute_likelihood_convolved_output_noise_fullspace(
          n=n, all_angles=posterior_space)

      # Get a spline interpolation
      convolv_posterior_spline = spinter.InterpolatedUnivariateSpline(
          posterior_space, convolved_posterior)
      normalization_convolv_posterior_spline = convolv_posterior_spline.integral(
          posterior_space[0], posterior_space[-1])

      # Compute the final convolved loglikelihoods for the actual thetas
      loglikelihoods[n] = np.log(
          convolv_posterior_spline(self.theta[n, self.theta_target_index[n]])
          .item()) - np.log(normalization_convolv_posterior_spline)

    return loglikelihoods

  def compute_loglikelihood_convolved_output_noise(self, precision=100):
    """Total summed loglikelihood, given convolved posterior with noise
    output."""
    return np.nansum(
        self.compute_loglikelihood_N_convolved_output_noise(
            precision=precision))

  def compute_likelihood_convolved_output_noise_fullspace(
      self, n=0, all_angles=None, precision=100, normalize=False):
    ...
    posterior = self.compute_loglikelihood_nt_fullspace(
        n=n,
        t=self.tc[n],
    ...
    convolved_posterior = np.roll(convolved_posterior,
                                  convolved_posterior.size / 2)

    if normalize:
      # Get a spline interpolation to compute the normalisation
      convolv_posterior_spline = spinter.InterpolatedUnivariateSpline(
          all_angles, convolved_posterior)
      normalization_convolv_posterior_spline = convolv_posterior_spline.integral(
  ...
  def compute_loglikelihood_nt_fullspace(self,
                                         n=0,
                                         t=0,
                                         all_angles=None,
                                         num_points=1000,
                                         normalize=False,
                                         should_exponentiate=False,
                                         remove_mean=False):
    """Computes and returns the loglikelihood/likelihood evaluated for a given
    datapoint n given time/item t on the entire space (e.g. [-pi,pi])."""
    ...
    # Compute the loglikelihood for all possible first feature
    for i in xrange(num_points):
      params = (curr_theta, self.NT[n], self.random_network, self.theta_gamma,
                self.theta_kappa, self.ATtcB[t], self.sampled_feature_index,
                self.mean_fixed_contrib[t], self.inv_covariance_fixed_contrib)
  ...
  def compute_likelihood_fullspace(self,
                                   n=0,
                                   all_angles=None,
                                   num_points=1000,
                                   normalize=False,
                                   remove_mean=False,
                                   should_exponentiate=False):
    """Computes and returns the (log)likelihood evaluated for a given datapoint
    on the entire space (e.g. [-pi,pi])."""
    ...
    # Compute the array
    for t in xrange(self.T):
      likelihood[t] = self.compute_loglikelihood_nt_fullspace(
          n=n,
          t=t,
          all_angles=all_angles,
          num_points=num_points,
          normalize=normalize,
          should_exponentiate=should_exponentiate,
  ...
  def plot_likelihood_variation_twoangles(self,
                                          index_second_feature=1,
                                          num_points=100,
                                          should_plot=True,
                                          should_return=False,
                                          should_exponentiate=False,
    ...
    for i in xrange(num_points):
      ...
      for j in xrange(num_points):
        ...
        params = (curr_theta, self.NT[n], self.random_network,
                  self.theta_gamma, self.theta_kappa, self.ATtcB[t],
                  self.sampled_feature_index, self.mean_fixed_contrib[t],
                  self.inv_covariance_fixed_contrib)
    ...
    if should_plot:
      ...
      for t in xrange(self.T):
        w = plt_patches.Wedge(
            (correct_angles[t, 0], correct_angles[t, index_second_feature]),
            0.25,
            0,
            360,
            0.10,
  ...
  def plot_likelihood_correctlycuedtimes(self,
                                         n=0,
                                         all_angles=None,
                                         num_points=500,
                                         should_plot=True,
    ...
    llh_2angles = self.compute_likelihood_fullspace(
        n=n,
        all_angles=all_angles,
        num_points=num_points,
        should_exponentiate=should_exponentiate,
    ...
    llh_2angles += 1.2 * np.arange(self.T) * np.abs(
        np.max(llh_2angles, axis=0) - np.mean(llh_2angles, axis=0))
    ...
    if should_plot:
      ...
      for t in xrange(self.T):
        ...
        ax_handle.axvline(
            x=self.data_gen.stimuli_correct[n, t, 0],
            color=lines[t].get_c())  # ax_handle[t] returns the plotted line

      # Put a dotted line at the current theta sample, for tc
      if show_current_theta:
        ax_handle.axvline(
            x=self.theta[n, self.theta_target_index[n]],
            color='k',
            linestyle="--")
    ...
    # Print the answers
    if debug:
      print "True angles: %s >> Inferred: %s" % (
          ' | '.join(
              ['%.3f' % x for x in self.data_gen.stimuli_correct[n, :, 0]]),
          ' | '.join(['%.3f' % x for x in all_angles[opt_angles]]))
  ...
  def plot_likelihood_convolved_output_noise(self,
                                             n=0,
                                             num_points=500,
                                             normalize=True,
                                             show_current_theta=True):
    """Plot the likelihood obtained by convolving the posterior with the noise
    output von mises."""

    all_angles = np.linspace(-np.pi, np.pi, num_points, endpoint=False)

    posterior = self.compute_likelihood_fullspace(
        n=n,
        all_angles=all_angles,
        normalize=normalize,
        should_exponentiate=True)[:, self.tc[n]]
    # posterior /= np.trapz(posterior, all_angles)
    noise = spst.vonmises.pdf(all_angles, self.kappa_output)

    convolved_posterior = self.compute_likelihood_convolved_output_noise_fullspace(
        n=n, all_angles=all_angles, normalize=normalize)

    # Plot now
    f, axes = plt.subplots(2, 1)
    axes.shape = 2
    axes[0].plot(all_angles, posterior, 'b', all_angles, noise, 'r')
    lines = axes[1].plot(all_angles, posterior, 'b', all_angles,
                         convolved_posterior, 'g')
    ...
    # Put a vertical line at the true answer
    axes[1].axvline(
        x=self.data_gen.stimuli_correct[n, self.tc[n], 0],
        color=lines[0].get_c())  # ax_handle[t] returns the plotted line
  ...
  def plot_likelihood_comparison(self, n=0):
    ...
    self.plot_likelihood_correctlycuedtimes(
        should_exponentiate=True, n=n, ax_handle=axes[0])
  ...
  def fit_mixture_model(self,
                        compute_responsibilities=False,
    ...
    if compute_responsibilities:
      params_fit['resp'] = em_circular_mixture_to_use.compute_responsibilities(
          *(self.collect_responses() + (params_fit, )))

    params_fit.setdefault('mixt_nontargets_sum',
                          np.sum(params_fit['mixt_nontargets']))

    return params_fit

  def compute_KL_mixture_model_responsibilites(self,
                                               dataset=None,
                                               data_em_fit=dict(),
                                               use_all_targets=False):
    """Compute the KL divergence between the mixture proportions of the model
    and the data.

    Give either the full experimental dataset, or the mixture model for
    the data:
      1) Assume that the dataset has keys: -> em_fits_nitems -> T
      2) Assume that the data_em_fit is a dictionary containing the following
        keys:
    ...
    model_mixtprop = np.array([
        model_em_fit[key]
        for key in ('mixt_target', 'mixt_nontargets_sum', 'mixt_random')
    ])
    data_mixtprop = np.array([
        data_em_fit[key]
        for key in ('mixt_target', 'mixt_nontargets', 'mixt_random')
    ])

    return utils.KL_div(model_mixtprop, data_mixtprop)

  def estimate_fisher_info_from_posterior(self,
                                          n=0,
                                          all_angles=None,
                                          num_points=500):
    """Look at the curvature of the posterior to estimate the Fisher
    Information."""

    if all_angles is None:
      all_angles = np.linspace(-np.pi, np.pi, num_points)

    log_posterior = self.compute_likelihood_fullspace(
        n=n,
        all_angles=all_angles,
        num_points=num_points,
        should_exponentiate=False,
        remove_mean=True)[:, self.tc[n]].T
  ...
  def estimate_fisher_info_from_posterior_avg(self,
                                              num_points=500,
                                              full_stats=False):
    ...
    for i in progress.ProgressDisplay(
        np.arange(self.N), display=progress.SINGLE_LINE):
      mean_FI[i] = self.estimate_fisher_info_from_posterior(
          n=i, all_angles=all_angles)

    if full_stats:
      return dict(
          mean=utils.nanmean(mean_FI),
          std=utils.nanstd(mean_FI),
          median=utils.nanmedian(mean_FI),
          all=mean_FI)
    else:
      return utils.nanmean(mean_FI)

  def estimate_fisher_info_from_posterior_avg_randomsubset(
      self, subset_size=1, num_points=500, full_stats=False):
    """Estimate the Fisher Information from the curvature of the posterior.
    ...
    for i in progress.ProgressDisplay(
        np.arange(subset_size), display=progress.SINGLE_LINE):
      mean_FI[i] = self.estimate_fisher_info_from_posterior(
          n=random_subset[i], all_angles=all_angles)
  ...
  def compute_covariance_theoretical(self,
                                     num_samples=1000,
                                     ignore_cache=False):
    """Compute and returns the theoretical covariance, found from KL
    minimization."""
  ...
  def estimate_fisher_info_theocov(self, use_theoretical_cov=True):
    """Compute the theoretical Fisher Information, using the KL-derived
    covariance matrix if desired."""

    if use_theoretical_cov:
      # Get the computed covariance
      computed_cov = self.compute_covariance_theoretical(
          num_samples=1000, ignore_cache=False)
    ...
    for n in xrange(self.N):
      fisher_info_all[n] = self.random_network.compute_fisher_information(
          stimulus_input=self.data_gen.stimuli_correct[n, self.tc[n]],
          inv_cov_stim=inv_cov_stim)
  ...
  def estimate_fisher_info_circulant(self):
    ...
    for n in xrange(self.N):
      fisher_info_all[
          n] = self.random_network.compute_fisher_information_circulant(
  ...
  def estimate_marginal_inverse_fisher_info_montecarlo(self):
    """Compute a Monte Carlo estimate of the Marginal Inverse Fisher
    Information.
  ...
  def estimate_precision_from_posterior(self, n=0, num_points=500):
    ...
    posterior = self.plot_likelihood_correctlycuedtimes(
        n=n,
        num_points=num_points,
        should_plot=False,
        should_return=True,
    ...
    try:
      ...
      precision_estimated = 1. / (
          -2. * np.log(np.abs(np.trapz(posterior * np.exp(1j * x), x))))
    except FloatingPointError:
      # print 'Overflow on n: %d' % n
  ...
  def estimate_precision_from_posterior_avg(self,
                                            num_points=500,
                                            full_stats=False):
    ...
    for i in progress.ProgressDisplay(
        np.arange(self.N), display=progress.SINGLE_LINE):
      precisions[i] = self.estimate_precision_from_posterior(
          n=i, num_points=num_points)

    if full_stats:
      return dict(
          mean=utils.nanmean(precisions),
          std=utils.nanstd(precisions),
          median=utils.nanmedian(precisions),
          all=precisions)
    else:
      return utils.nanmean(precisions)

  def estimate_precision_from_posterior_avg_randomsubset(
      self, subset_size=1, num_points=1000, full_stats=False):
    """Estimate the precision from the posterior.
    ...
    for i in progress.ProgressDisplay(
      ...
      precisions[i] = self.estimate_precision_from_posterior(
          n=random_subset[i], num_points=num_points)
  ...
  def estimate_truevariance_from_posterior_avg(self,
                                               all_angles=None,
                                               num_points=500,
                                               full_stats=False):
    ...
    for i in progress.ProgressDisplay(
      ...
      truevariances[i] = self.estimate_truevariance_from_posterior(
          n=i, all_angles=all_angles, num_points=num_points)

    if full_stats:
      return dict(
          mean=utils.nanmean(truevariances),
          std=utils.nanstd(truevariances),
          median=utils.nanmedian(truevariances),
          all=truevariances)
    else:
      return utils.nanmean(truevariances)

  def estimate_gaussianity_from_samples(self,
                                        significance_level=0.025,
                                        n=0,
                                        num_samples=500,
                                        num_repetitions=1,
                                        selection_method='median'):
    """Take samples of theta for a particular datapoint, and estimate their
    gaussianity."""
    ...
    for repet_i in xrange(num_repetitions):
      ...
      # Get samples
      all_samples[repet_i] = self.sample_theta(
          return_samples=True, subset_theta=[n])[0]
      ...
      all_gaussianity[repet_i] = spst.normaltest(
          all_samples[repet_i])[1] >= significance_level
  ...
  def plot_comparison_samples_fit_posterior(self,
                                            ...
                                            samples=None,
                                            num_points=1000):
    """Plot a series of samples (usually from theta), associated with the
    posterior generating them and a gaussian fit.
  ...
  def plot_bias_close_feature(self, dataio=None, markersize=3):
    """Reproduces plots from load_experimental_data, to see the difference
    ...
    dist_target_nontarget_recalled = np.abs(
        utils.wrap_angles(target_recall_feature - nontarget_recall_feature))
    ...
    indices_filter_other_side = (bias_to_nontarget >
                                 dist_target_nontarget_recalled)
    ratio_response_close_to_nontarget[indices_filter_other_side] = (
        bias_to_nontarget[indices_filter_other_side] /
        (dist_target_nontarget_recalled[indices_filter_other_side] +
         bias_to_target[indices_filter_other_side]))

    f, ax = plt.subplots(2, 2, figsize=(8, 8))
    ax[0, 0].plot(
        dist_target_nontarget_torus,
        bias_to_nontarget,
        'o',
  ...
  def run(self, iterations=10, verbose=True):
    ...
    if verbose:
      print "Initialisation: likelihoods = y %.3f, z %.3f, joint: %.3f" % (
          log_y[0], log_z[0], log_joint[0])
    ...
    for i in xrange(iterations):
      ...
      # Get the likelihoods
      (log_y[i + 1], log_z[i + 1],
       log_joint[i + 1]) = self.compute_all_loglike()

      # Print report
      if verbose:
        print "Sample %d: likelihoods = y %.3f, z %.3f, joint: %.3f" % (
            i + 1, log_y[i + 1], log_z[i + 1], log_joint[i + 1])
  ...
  def compute_errors_alltargets_histograms(self, bins=41, norm='density'):
    """Estimates the histograms of Targets and Nontargets errors. Also returns
    the ECDF, for nicer plottings and further K-S 2-samples tests.
    ...
    outputs['targets'] = utils.empirical_cdf_samples(
        errors_targets, bins=bins, norm=norm)
    if self.T > 1:
  ...
  def get_precision(self, remove_chance_level=False, correction_theo_fit=1.0):
    """Compute the precision, inverse of the variance of the errors. This is
    our target metric.

    Approximately half of Fisher Information estimate, because of doubly
    stochastic process and near-gaussianity.
    """

    # Compute precision
    precision = utils.compute_angle_precision_from_std(
        self.compute_angle_error()['std'], square_precision=True)
  ...
  def print_comparison_inferred_groundtruth(self, show_nontargets=True):
    ...
    (stats, angle_errors, groundtruth) = self.compute_angle_error(
        return_groundtruth=True, return_errors=True)
    ...
    if show_nontargets:
      ...
      print "Target " + ''.join([
          "\t NT %d " % x for x in (np.arange(nontargets.shape[1]) + 1)
      ]) + "\t Inferred \t Error"

      for i in xrange(self.N):
        print "% 4.3f" % (groundtruth[i]) + ''.join([
            "\t\t% 4.3f" % x for x in nontargets[i]
        ]) + "\t\t % 4.3f \t % 4.3f" % (self.theta[i, 0], angle_errors[i])
    else:
      print "Target \t Inferred \t Error"
      for i in xrange(self.N):
        print "% 4.3f \t\t % 4.3f \t % 4.3f" % (self.theta[i, 0],
                                                groundtruth[i],
                                                angle_errors[i])
  ...
  def plot_histogram_errors(self,
                            bins=20,
                            in_degrees=False,
                            norm='max',
                            nice_xticks=False,
    ...
    utils.hist_angular_data(
        errors,
        bins=bins,
        title='Errors between response and target',
        norm=norm,
  ...
  def plot_histogram_responses(self,
                               bins=20,
                               in_degrees=False,
                               norm='max',
                               show_angles=False,
  ...
  def plot_histogram_bias_nontarget(self,
    ...
    if plot_best_nontarget:
      ...
      errors_best_nontarget = utils.wrap_angles(
          (responses[:, np.newaxis] - nontargets))
      errors_best_nontarget = errors_best_nontarget[
          np.arange(errors_best_nontarget.shape[0]),
          np.argmin(np.abs(errors_best_nontarget), axis=1)]

      utils.hist_samples_density_estimation(
          errors_best_nontarget,
          bins=angle_space,
          title='Errors between response and best non-target, N=%d' % (self.T),
          filename='hist_bias_bestnontarget_%ditems_{label}_{unique_id}.pdf' %
          (self.T),
          dataio=dataio,
          ax_handle=ax_handle_bestnontarget)
