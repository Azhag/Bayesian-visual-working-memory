...
# encoding: utf-8
"""
launchers_fisherinformation_deprecated.py
...
def launcher_do_fisher_information_estimation(args):
    ...
    if args.subaction == 'M_dependence':
        ...
        for i, M in enumerate(M_space):
            ...
            if code_type == 'conj':
                random_network = randomfactorialnetwork.RandomFactorialNetwork.create_full_conjunctive(M, R=R, scale_moments=(rc_scale, 0.00001), ratio_moments=(1.0, 0.00001))
            elif code_type == 'feat':
                random_network = randomfactorialnetwork.RandomFactorialNetwork.create_full_features(M, R=R, scale=rc_scale, ratio=40.)
                ...
                random_network = randomfactorialnetwork.RandomFactorialNetwork.create_mixed(M, R=R, ratio_feature_conjunctive=ratio_conj, conjunctive_parameters=conj_params, feature_parameters=feat_params)
            elif code_type == 'wavelet':
                random_network = randomfactorialnetwork.RandomFactorialNetwork.create_wavelet(M, R=R, scales_number=5)
            else:
        ...
        for i, num_samples in enumerate(samples_space):
            ...
            if code_type == 'conj':
                random_network = randomfactorialnetwork.RandomFactorialNetwork.create_full_conjunctive(M, R=R, scale_moments=(rc_scale, 0.0001), ratio_moments=(1.0, 0.0001))
            elif code_type == 'feat':
                random_network = randomfactorialnetwork.RandomFactorialNetwork.create_full_features(M, R=R, scale=rc_scale, ratio=40.)
            elif code_type == 'mixed':
                conj_params = dict(scale_moments=(rc_scale, 0.001), ratio_moments=(1.0, 0.0001))
...
def init_everything(parameters):
    ...
    if parameters['code_type'] == 'conj':
        random_network = randomfactorialnetwork.RandomFactorialNetwork.create_full_conjunctive(parameters['M'], R=parameters['R'], scale_moments=(parameters['rc_scale'], 0.0001), ratio_moments=(1.0, 0.0001))
    elif parameters['code_type'] == 'feat':
        random_network = randomfactorialnetwork.RandomFactorialNetwork.create_full_features(parameters['M'], R=parameters['R'], scale=parameters['rc_scale'], ratio=40.)
        ...
        random_network = randomfactorialnetwork.RandomFactorialNetwork.create_mixed(parameters['M'], R=parameters['R'], ratio_feature_conjunctive=parameters['ratio_conj'], conjunctive_parameters=conj_params, feature_parameters=feat_params)
    elif parameters['code_type'] == 'wavelet':
        random_network = randomfactorialnetwork.RandomFactorialNetwork.create_wavelet(parameters['M'], R=parameters['R'], scales_number=5)
    else:
        raise ValueError('Code_type is wrong!')
