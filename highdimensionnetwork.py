...
class HighDimensionNetwork(object):
  ...
  def assign_prefered_stimuli(self,
                              tiling_type='conjunctive',
    ...
    if specific_neurons.size != 0:
      ...
      self.mask_neurons_unset = np.any(
          np.isnan(self.neurons_preferred_stimulus), 1)
  ...
  def assign_prefered_stimuli_conjunctive(self, neurons_indices):
    ...
    self.neurons_preferred_stimulus[neurons_indices[:new_stim.shape[
        0]]] = new_stim
  ...
  def assign_prefered_stimuli_features(self, neurons_indices):
    ...
    coverage_1D = np.linspace(
        -np.pi + np.pi / N, np.pi + np.pi / N, N, endpoint=False)

    for r in xrange(self.R):
      self.neurons_preferred_stimulus[neurons_indices[N * r:N * (
          r + 1)], :] = center
      self.neurons_preferred_stimulus[neurons_indices[N * r:N * (
          r + 1)], r] = coverage_1D
  ...
  def assign_aligned_eigenvectors(self,
                                  scale=1.0,
                                  ratio=1.0,
                                  scaled_dimension=1,
                                  specific_neurons=None,
  ...
  def precompute_parameters(self, specific_neurons=None):
    ...
    self.compute_normalising_constant_bivariatefisher(
  ...
  def get_network_response(self, stimulus_input=None, specific_neurons=None):
    ...
    return self.get_network_response_bivariatefisher_callback(
        stimulus_input, specific_neurons=specific_neurons)

  def get_network_response_bivariatefisher(self,
                                           stimulus_input,
                                           specific_neurons=None):
    '''
    ...
    if self.get_network_response_opt is not None and specific_neurons is None:
      ...
      output = np.exp(
          np.sum(self.neurons_sigma[specific_neurons] * np.cos(dmu),
                 axis=-1)) / self.normalisation[specific_neurons]

      output[self.mask_neurons_unset[specific_neurons]] = 0.0

    return output

  def get_network_response_bivariatefisher_maxoutput(self,
                                                     stimulus_input,
                                                     specific_neurons=None):
    '''
            Compute the response of the network.
    ...
    dmu = stimulus_input - self.neurons_preferred_stimulus[specific_neurons]
    output = np.exp(-np.sum(
        self.neurons_sigma[specific_neurons] * (1. - np.cos(dmu)), axis=-1))
  ...
  def get_network_response_large_kappa_safe(self, stimulus_input):
    ...
    for r in xrange(self.R):
      ...
      output[index_fish] *= np.exp(self.neurons_sigma[index_fish, r] * np.cos(
          (stimulus_input[r] - self.neurons_preferred_stimulus[index_fish, r]
           ))) / self.normalisation_fisher_all[index_fish, r]

      output[index_gauss] *= np.exp(
          -0.5 * self.neurons_sigma[index_gauss, r] *
          (stimulus_input[r] - self.neurons_preferred_stimulus[index_gauss, r]
           )**2.) * self.normalisation_gauss_all[index_gauss, r]
  ...
  def get_derivative_network_response(self,
    ...
    dmu_specific_feature = stimulus_input[derivative_feature_target] - self.neurons_preferred_stimulus[:,
                                                                                                       derivative_feature_target]

    der_f = self.neurons_sigma[:, derivative_feature_target] * np.sin(
        dmu_specific_feature) * self.get_network_response(stimulus_input)
  ...
  def compute_network_response_statistics(self,
                                          num_samples=5000,
                                          ignore_cache=False):
    '''
    ...
    if ignore_cache or self.network_response_statistics is None:
      ...
      # Cache them
      self.network_response_statistics = {
          'mean': computed_mean,
          'cov': computed_cov
      }
  ...
  def collect_network_responses(self, num_samples=5000):
    ...
    for i in progress.ProgressDisplay(
        xrange(num_samples), display=progress.SINGLE_LINE):
  ...
  def get_neuron_response(self, neuron_index, stimulus_input):
    ...
    return self.get_network_response(
        stimulus_input, specific_neurons=np.array([neuron_index]))
  ...
  def sample_multiple_network_response(self, stimuli_input, sigma=0.2):
    ...
    for i in xrange(nb_samples):
      net_samples[i] = self.sample_network_response(
          stimuli_input[i], sigma=sigma)
  ...
  def compute_covariance_KL(self,
                            num_samples=5000,
                            sigma_x=0.1,
                            sigma_y=0.0001,
    ...
    network_response_statistics = self.compute_network_response_statistics(
        num_samples=num_samples, ignore_cache=ignore_cache)

    # !!! The actual Fisher Information computation !!!
    covariance = (
        T - 1) * beta**2. * network_response_statistics['cov'] + T * (
            sigma_x**2. + sigma_y**2.) * np.eye(
                self.M) + sigma_baseline**2. * np.eye(self.M)
  ...
  def compute_marginal_inverse_FI(self,
    ...
    marginal_fi_dict = dict(inv_FI=0, inv_FI_std=0, FI=0, FI_std=0)
    inv_FI_cum = 0
    FI_cum = 0
    FI_cum_var = 0
    inv_FI_cum_var = 0
    ...
    for i in xrange(max_n_samples):
      ...
      if i > min_num_samples_std:
        inv_FI_cum_var += (inv_FI_sample - inv_FI_cum * max_n_samples /
                           (i + 1.))**2. / float(max_n_samples)
        FI_cum_var += (FI_sample - FI_cum * max_n_samples /
                       (i + 1.))**2. / float(max_n_samples)

      # Compute current running averages
      marginal_fi_dict['inv_FI'] = inv_FI_cum * max_n_samples / (i + 1.)
      marginal_fi_dict['inv_FI_std'] = np.sqrt(inv_FI_cum_var * max_n_samples /
                                               (i + 1.)) / np.sqrt(i + 1)
      marginal_fi_dict['FI'] = FI_cum * max_n_samples / (i + 1.)
      marginal_fi_dict['FI_std'] = np.sqrt(FI_cum_var * max_n_samples /
                                           (i + 1.)) / np.sqrt(i + 1)
  ...
  def compute_fisher_information_circulant(self, stimulus_input, cov_stim):
    ...
    IF_fourier = np.abs(
        np.dot(der_f_tilde.conj(), np.dot(np.diag(c_tilde**-1),
                                          der_f_tilde))) / der_f.size
  ...
  def init_feature_cover_matrices_2d(self, precision=20, endpoint=True):
    ...
    feature_space1 = self.init_feature_space(
        precision=precision, endpoint=endpoint)
    feature_space2 = self.init_feature_space(
        precision=precision, endpoint=endpoint)
  ...
  def plot_coverage_feature_space(self,
    ...
    if facecolor is None or facecolor == 'rand':
      pointcolor = 'k'
    else:
      pointcolor = facecolor

    ax.plot(
        self.neurons_preferred_stimulus[~self.mask_neurons_unset, 0],
        self.neurons_preferred_stimulus[~self.mask_neurons_unset, 1],
        'o',
        markersize=2,
        color=pointcolor)
    ...
    for e in ells:
      ...
      elif (facecolor is False or facecolor is None or facecolor == 'none'
            or facecolor == 'None'):
  ...
  def plot_mean_activity(self, precision=100, specific_neurons=None):
    ...
    (mean_activity, feature_space1, feature_space2) = self.get_mean_activity(
        precision=precision,
        specific_neurons=specific_neurons,
        return_axes_vect=True)
  ...
  def plot_coverage_preferred_stimuli_3d(self, axes=(0, 1)):
    ...
    utils.scatter3d_torus(self.neurons_preferred_stimulus[:, axes[0]],
                          self.neurons_preferred_stimulus[:, axes[1]])

  def plot_neuron_activity_3d(self,
                              neuron_index=0,
                              precision=20,
                              axes=(0, 1),
                              weight_deform=0.5,
    ...
    coverage_1D = self.init_feature_space(precision)
    activity = self.get_neuron_activity(
        neuron_index, precision=precision, axes=axes)
  ...
  def plot_mean_activity_3d(self,
                            precision=20,
                            axes=(0, 1),
                            specific_neurons=None,
                            weight_deform=0.5):
  ...
  @classmethod
  def create_full_conjunctive(cls,
                              M,
                              R=2,
                              rcscale=None,
    ...
    if debug:
      print "create conjunctive network, R=%d, M=%d, autoset: %d" % (
    ...
    if autoset_parameters:
      ...
      rcscale = cls.compute_optimal_rcscale(
          M, R, population_code_type='conjunctive')
  ...
  def create_mixed(cls,
                   M,
                   R=2,
                   ratio_feature_conjunctive=0.5,
                   conjunctive_parameters=None,
    ...
    if autoset_parameters:
      ...
      if rn.conj_subpop_size > 0:
        conj_scale = cls.compute_optimal_rcscale(
            rn.conj_subpop_size, R, population_code_type='conjunctive')
      if rn.feat_subpop_size > 0:
        feat_scale = utils.stddev_to_kappa(np.pi)
        feat_ratio = cls.compute_optimal_rcscale(
            rn.feat_subpop_size, R,
            population_code_type='feature') / feat_scale

    print "Population sizes: ratio: %.2f conj: %d, feat: %d, autoset: %d" % (
        ratio_feature_conjunctive, rn.conj_subpop_size, rn.feat_subpop_size,
        autoset_parameters)

    # Create the conjunctive subpopulation
    rn.assign_prefered_stimuli(
        tiling_type='conjunctive',
        reset=True,
        specific_neurons=np.arange(rn.conj_subpop_size))
    rn.assign_aligned_eigenvectors(
        scale=conj_scale,
        ratio=1.0,
        specific_neurons=np.arange(rn.conj_subpop_size),
    ...
    # Assign centers
    rn.assign_prefered_stimuli(
        tiling_type='features',
        specific_neurons=np.arange(rn.conj_subpop_size, M))
    # Now assign scales
    ...
    for r in xrange(rn.R):
      rn.assign_aligned_eigenvectors(
          ...
          ratio=feat_ratio,
          scaled_dimension=r,
          specific_neurons=np.arange(rn.conj_subpop_size + r * feat_sub_M,
                                     rn.conj_subpop_size +
                                     (r + 1) * feat_sub_M))
...
def test_optimised_network_response():
  ...
  M = int(50 * R + 10**R)
  ratio = 0.1
  rn = HighDimensionNetwork.create_mixed(
      M,
      R=R,
      ratio_feature_conjunctive=ratio,
  ...
  for curr_angles in rnd_angles:
    all_correct = all_correct and np.allclose(
        rn.get_network_response_bivariatefisher(
            curr_angles, params=dict(opt=True)),
        rn.get_network_response_bivariatefisher(
            curr_angles, params=dict(opt=False)))
...
def test_large_kappa_network_response():
  ...
  ratio = 1.0
  rn = HighDimensionNetwork.create_mixed(
      M, R=R, ratio_feature_conjunctive=ratio, autoset_parameters=True)
