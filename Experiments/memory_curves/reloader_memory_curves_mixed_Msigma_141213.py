...
def plots_memory_curves(data_pbs, generator_module=None):
    ...
    bays09_experimental_mixtures_mean_compatible = np.c_[bays09_experimental_mixtures_mean[:,:2], bays3, bays09_experimental_mixtures_mean[:, 2], bays5, bays09_experimental_mixtures_mean[:, -1]]
    ...
    dist_diff_precision_experim = np.sum(np.abs(result_all_precisions_mean[..., :memory_experimental_kappa.size] - memory_experimental_precision)**2., axis=-1)
    dist_diff_emkappa_experim = np.sum(np.abs(result_em_fits_mean[..., 0, :memory_experimental_kappa.size] - memory_experimental_kappa)**2., axis=-1)
    ...
    def mem_plot_kappa(sigmax_i, M_i, mem_exp_kappa):
        ax = utils.plot_mean_std_area(T_space[:mem_exp_kappa.size], mem_exp_kappa, np.zeros(mem_exp_kappa.size), linewidth=3, fmt='o-', markersize=8, label='Experimental data')
        ...
        ax.set_title('M %d, sigmax %.2f' % (M_space[M_i], sigmax_space[sigmax_i]))
        ax.legend()
        ax.set_xlim([0.9, mem_exp_kappa.size+0.1])
        ax.set_xticks(range(1, mem_exp_kappa.size+1))
        ax.set_xticklabels(range(1, mem_exp_kappa.size+1))
    ...
    if plot_best_memory_curves:
        ...
        for axis1_i, best_axis2_i in enumerate(best_axis2_i_all):
            mem_plot_precision(best_axis2_i, axis1_i, memory_experimental_precision)

        # Best kappa fit
        best_axis2_i_all = np.argmin(dist_diff_emkappa_experim, axis=1)

        for axis1_i, best_axis2_i in enumerate(best_axis2_i_all):
            mem_plot_kappa(best_axis2_i, axis1_i, memory_experimental_kappa)
        ...
        for axis1_i, best_axis2_i in enumerate(best_axis2_i_all):
            mem_plot_kappa(best_axis2_i, axis1_i, bays09_experimental_mixtures_mean_compatible[0])
            # em_plot(best_axis2_i, axis1_i)
