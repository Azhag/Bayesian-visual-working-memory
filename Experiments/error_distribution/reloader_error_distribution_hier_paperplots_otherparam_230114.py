...
def plots_errors_distribution(data_pbs, generator_module=None):
    ...
    caching_bootstrap_filename = os.path.join(generator_module.pbs_submission_infos['simul_out_dir'], 'outputs', 'cache_bootstrap_errordistrib_hier_sigmaxT_paper.pickle')
    ...
    if plot_persigmax:
        ...
        for T_i, T in enumerate(T_space_filtered):
            ...
            utils.hist_angular_data(utils.dropnan(errors_targets[T-1]), bins=angle_space, norm='density', ax_handle=axes1[T_i], pretty_xticks=True)
            ...
            if T > 1:
                ...
                utils.hist_angular_data(utils.dropnan(errors_nontargets[T-1, :, :(T-1)]), bins=angle_space, title='N=%d' % (T), norm='density', ax_handle=axes2[T_i-1], pretty_xticks=True)
                ...
                # axes2[T_i-1].text(0.03, 0.96, "Vtest pval: %.2f" % (result_pval_vtest_nontargets[T_i]), transform=axes2[T_i - 1].transAxes, horizontalalignment='left', fontsize=12)
                axes2[T_i-1].text(0.03, 0.94, "$p=%.1f$" % (result_pvalue_bootstrap_sum[T-2]), transform=axes2[T_i - 1].transAxes, horizontalalignment='left', fontsize=18)
    ...
    if savedata:
        ...
        dataio.make_link_in_directory(source_file=os.path.split(this_file)[-1], output_dir=os.path.join(generator_module.pbs_submission_infos['simul_out_dir'], 'outputs/'))
