    ...
    if plot_persigmax:
        ...
        for T_i, T in enumerate(T_space_filtered):
            ...
            if T > 1:
                ...
                result_pval_vtest_nontargets[T_i] = utils.V_test(utils.dropnan(errors_nontargets[T_i, :, :T_i]))['pvalue']

                print result_pval_vtest_nontargets[T_i]
                ...
                axes2[T_i-1].text(0.03, 0.94, "$p=%.1f$" % (result_pvalue_bootstrap_sum[T_i]), transform=axes2[T_i - 1].transAxes, horizontalalignment='left', fontsize=18)
