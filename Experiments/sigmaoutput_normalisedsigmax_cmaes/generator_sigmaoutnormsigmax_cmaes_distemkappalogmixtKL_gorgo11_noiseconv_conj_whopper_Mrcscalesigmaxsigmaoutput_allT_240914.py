...
run_label = 'sigmaoutnormsigmax_cmaes_distemkappalogmixtKL_gorgo11_noiseconv_conj_whopper_Mrcscalesigmaxsigmaoutput_allT_repetitions{num_repetitions}_240914'
...
pbs_submission_infos = dict(description='Fit experiments (gorgo11), using distem_logkappa_mixtKL_gorgo11 ResultComputation), using the CMA-ES code. Looks at all t<=T here. Changes M, rc scale (could be unoptimal now), sigmax and sigma_output. Get samples and look at the mixture proportions and kappa. Tries to combine them directly, but could do with some normalisation...',
...
rcscale_range   =   dict(low=0.001, high=50.0, dtype=float)
sigmaoutput_range =   dict(low=0.0, high=0.5, dtype=float)
M_range           =   dict(low=5, high=400, dtype=int)

dict_parameters_range =   dict(sigma_output=sigmaoutput_range, rc_scale=rcscale_range, sigmax=sigmax_range, M=M_range)
