...
dataset_infos = dict(label='Study misbindings, by computing an average posterior for fixed stimuli. Check the distribution of errors as well. Uses a Hierarchical population, vary ratio_conj to see what happens. Limit to squared ratio_subpop_nb only. Plots already done before, but lets try to redo them',
                     files="%s/%s*.npy" % (generator_module.pbs_submission_infos['simul_out_dir'], generator_module.pbs_submission_infos['other_options']['label'].split('{')[0]),
                     launcher_module=generator_module,
                     loading_type='args',
                     parameters=['M', 'M_layer_one'],
                     variables_to_load=['result_all_log_posterior', 'result_all_thetas'],
                     variables_description=['Log posterior for multiple M/M_lower', 'Thetas (responses) for multiple M/M_lower'],
