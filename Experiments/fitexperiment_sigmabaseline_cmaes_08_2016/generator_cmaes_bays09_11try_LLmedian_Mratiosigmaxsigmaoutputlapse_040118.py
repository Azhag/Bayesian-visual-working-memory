...
run_label = 'cmaes_bays09_11try_llmedian_Mratiosigmaxsigmaoutputlapserate_repetitions{num_repetitions}_040118'
...
pbs_submission_infos = dict(
    description='''Fit experiments (bays09), using dist_ll_median_allt ResultComputation), using the CMA-ES code. Using new fixed Covariance matrix for Sampler, should change N=1 case most.

    Changes M, ratio_conj, sigmax, sigma output, lapse rate.
    Looks at all t<=T. Use full LL score, all datapoints.

    USING SIGMA OUTPUT AGAIN TO CHECK.
    ...
    other_options=dict(
        ...
        bic_K=5,
        ratio_conj=0.5,
        session_id='cmaes_11try_Mratiosigmaxlapsesigmaout_bays09',
...
sigmaoutput_range = dict(low=0.0,
                         high=1.,
                         x0=0.1,
                         scaling=cma_sigma0/3.,
                         dtype=float,
...
dict_parameters_range = dict(M=M_range,
                             ...
                             sigmax=sigmax_range,
                             sigma_output=sigmaoutput_range
                             )
# ============================================================================
