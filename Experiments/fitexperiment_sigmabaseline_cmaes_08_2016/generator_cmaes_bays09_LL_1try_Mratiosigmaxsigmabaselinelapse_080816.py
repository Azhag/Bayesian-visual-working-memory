...
run_label = 'cmaes_bays09_ll_1try_Mratiosigmaxsigmabaselinelapserate_repetitions{num_repetitions}_080816'
...
pbs_submission_infos = dict(
    description=
    'Fit experiments (bays09), using dist_ll_allt ResultComputation), using the CMA-ES code. Now with sigma_baseline instead of sigma_output. Looks at all t<=T here. Changes M, ratio_conj, sigmax, sigma baseline, lapse rate. Only looks at LL score, all datapoints. Looking for best meaningful parameter bounds now. Using new fixed Covariance matrix for Sampler, should change N=1 case most.',
...
best_parameters_seen = dict(
    result=np.nan,
    job_name='',
    parameters=None,
    submit_best=False,
    pbs_submission_infos_copy=copy.deepcopy(pbs_submission_infos))
    ...
    if not np.any(np.isnan(job.get_result())) and (
        ...
        if parameters.get('submit_best', False):
            ...
            try:
                ...
                curr_params_label = '_'.join([
                    "%s%.2f" % (k.replace('_', ''), v)
                    for k, v in parameters['best_parameters'].iteritems()
                ])
                ...
                pbs_submission_infos_copy['other_options'].update(
                    dict(
                        action_to_do=
                        'launcher_do_memory_curve_marginal_fi_withplots_live',
                        subaction='collect_responses',
                        inference_method='sample',
                        N=300,
                        T=6,
                        num_samples=300,
                        output_directory=os.path.join(simul_out_dir,
                                                      'outputs'),
                        selection_method='last',
                        num_repetitions=10,
                        burn_samples=200,
                        stimuli_generation='random',
                        stimuli_generation_recall='random',
                        session_id='cmaes_bays09_1try_rerun_080816',
                        result_computation='filenameoutput',
                        label='%s_cmaes_bays09_1try_080816' %
                        (curr_params_label)))
                pbs_submission_infos_copy['walltime'] = '40:00:00'
                pbs_submission_infos_copy['submit_label'] = 'bestparam_rerun'

                submit_pbs = submitpbs.SubmitPBS(
                    pbs_submission_infos=pbs_submission_infos_copy, debug=True)

                # Extract the parameters to try
                best_params_resend = [
                    utils.subdict(job.experiment_parameters,
                ...
                submission_parameters_dict = dict(
                    pbs_submission_infos=pbs_submission_infos_copy,
                    submit_jobs=submit_jobs,
                    wait_jobs_completed=False)
                submit_pbs.submit_minibatch_jobswrapper(
                    best_params_resend, submission_parameters_dict)
...
data_io = dataio.DataIO(
    label='cmaes_alliter_tracking',
...
def cma_iter_plot_scatter3d_candidates(all_variables, parameters=None):
    ...
    if parameters['dataio'] is not None:
        parameters['dataio'].save_variables_default(
            locals(), ['candidates', 'fitness', 'parameter_names_sorted'])
        parameters['dataio'].make_link_output_to_dropbox(
            dropbox_current_experiment_folder=
            'fitexperiment_sigmabaseline_cmaes_08_2016')
...
cma_iter_callback_function_infos = dict(
    function=cma_iter_plot_scatter3d_candidates,
