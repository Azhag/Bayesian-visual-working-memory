...
run_label = 'cmaes_gorgo11sequential_4try_llmedian_Mratiosigxsigbaselapsealpha_rep{num_repetitions}_050118'
...
pbs_submission_infos = dict(
    description=
    '''Fit sequential experiment (gorgo11 sequential), using dist_ll92_allt ResultComputation), using the CMA-ES code. Now with sigma_baseline instead of sigma_output. Using new fixed Covariance matrix for Sampler, should change N=1 case most.
...
alpha_range = dict(low=0.5,
                   high=1.0,
                   x0=0.8,
                   scaling=cma_sigma0/6.,
                   dtype=float
                   )
M_range = dict(low=25,
               high=400,
