...
run_label = 'cmaes_bays09_ll97_5try_Mratiosigmaxsigmabaselinelapserate_repetitions{num_repetitions}_170816'
...
pbs_submission_infos = dict(description='''Fit experiments (bays09), using dist_ll97_allt ResultComputation), using the CMA-ES code. Now with sigma_baseline instead of sigma_output. Using new fixed Covariance matrix for Sampler, should change N=1 case most.
  ...
  ''',
                            ...
                            other_options=dict(action_to_do='launcher_do_fitexperiment_allmetrics',
                                               ...
                                               bic_K=5,
                                               ratio_conj=0.5,
                                               session_id='cmaes_5try_Mratiosigmaxlapsesigmabase_bays09',
                                               result_computation='dist_ll97_allt',
                                               M=100,
                                               sigmax=0.1,
        ...
        if parameters.get('submit_best', False):
            ...
            try:
                ...
                pbs_submission_infos_copy['other_options'].update(dict(
                    ...
                    stimuli_generation='random',
                    stimuli_generation_recall='random',
                    session_id='cmaes_bays09_5try_rerun_080816',
                    result_computation='filenameoutput',
                    label='%s_cmaes_bays09_5try_080816' % (curr_params_label)
                ))
