...
def plots_fisherinfo_singleitem(data_pbs, generator_module=None):
    ...
    if plot_metric_comparison:
        values_bars = np.array([np.mean(result_FI_rc_precision_mult), np.mean(result_FI_rc_theo_mult[0]), np.mean(result_FI_rc_theo_mult[1]), np.mean(result_FI_rc_samples_all), np.mean(result_FI_rc_curv_all)])
        values_bars_std = np.array([np.std(result_FI_rc_precision_mult), np.std(result_FI_rc_theo_mult[0]), np.std(result_FI_rc_theo_mult[1]), np.std(result_FI_rc_samples_all), np.std(result_FI_rc_curv_all)])
