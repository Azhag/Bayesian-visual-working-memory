...
def plots_fitexp_random(data_pbs, generator_module=None):
    '''
    ...
    fitexperiment_parameters = data_pbs.loaded_data['datasets_list'][0]['fitexperiment_parameters']

    result_fitexperiments_flat = np.array(data_pbs.dict_arrays['result_fitexperiments']['results_flat'])
    result_fitexperiments_flat_all = np.array(data_pbs.dict_arrays['result_fitexperiments_all']['results_flat'])
    ...
    result_bic_sumT = np.nansum(utils.nanmean(result_fitexperiments_flat[:, :, 0], axis=-1), axis=-1)
    ...
    if plots_scatters:
        ...
        def plot_scatter(all_vars, result_dist_to_use_name, title='', log_color=True, downsampling=1, label_file=''):
            ...
            ax.scatter(result_parameters_flat[:, 0], result_parameters_flat[:, 1], s=size_normal_points, c=np.log(result_dist_to_use))
            ax.set_xlabel(parameter_names_sorted[0])
            ax.set_ylabel(parameter_names_sorted[1])
            ax.set_title(title)

            best_points_result_dist_to_use = np.argsort(result_dist_to_use)[:nb_best_points]
            ax.scatter(result_parameters_flat[best_points_result_dist_to_use, 0], result_parameters_flat[best_points_result_dist_to_use, 1], c='r', s=size_best_points)
            print "Best points, %s:" % title
            print '\n'.join(['ratio %.4f, sigmax %.4f:  %f' % (result_parameters_flat[i, 0], result_parameters_flat[i, 1], result_dist_to_use[i]) for i in best_points_result_dist_to_use])

            if savefigs:
                dataio.save_current_figure('scatter_%s%s_{label}_{unique_id}.pdf' % (result_dist_to_use_name, label_file))

                if savemovies:
                    try:
                        utils.rotate_plot3d(ax, dataio.create_formatted_filename('scatter_%s%s_{label}_{unique_id}.mp4' % (result_dist_to_use_name, label_file)), bitrate=8000, min_duration=8)
                        utils.rotate_plot3d(ax, dataio.create_formatted_filename('scatter_%s%s_{label}_{unique_id}.gif' % (result_dist_to_use_name, label_file)), nb_frames=30, min_duration=8)
                    except Exception:
                        # Most likely wrong aggregator...
                        print "failed when creating movies for ", result_dist_to_use_name

                # ax.view_init(azim=90, elev=10)
                # dataio.save_current_figure('scatter_view2_%s%s_{label}_{unique_id}.pdf' % (result_dist_to_use_name, label_file))
        ...
        plot_scatter(locals(), 'result_bic_sumT', 'BIC, summed T')
...
dataset_infos = dict(label='Fitting of experimental data. All experiments. Random sampling of parameter space. Perhaps too big, be careful...',
                     ...
                     variables_description=['T x {BIC, LL, LL90} x num_repetitions', 'T x {BIC, LL, LL90} x experiment x num_repetitions', 'Same but under noise convolved posterior', 'Same, per experiment'],
                     post_processing=plots_fitexp_random,
