...
def plots_fitexp_random(data_pbs, generator_module=None):
    ...
    if plots_scatters:
        ...
        def plot_scatter(all_vars, result_dist_to_use_name, title='', log_color=True, downsampling=1, label_file=''):
            ...
            print '\n'.join(['M %d, ratio %.4f, sigmax %.4f:  %f' % (result_parameters_flat[i, 0], result_parameters_flat[i, 1], result_parameters_flat[i, 2], result_dist_to_use[i]) for i in best_points_result_dist_to_use])
...
dataset_infos = dict(label='Fitting of experimental data. All experiments. Random sampling of parameter space. Perhaps too big, be careful...',
                     ...
                     variables_description=['T x {BIC, LL, LL90} x num_repetitions', 'T x {BIC, LL, LL90} x experiment x num_repetitions'],
