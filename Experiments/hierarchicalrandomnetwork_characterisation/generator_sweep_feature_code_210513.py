...
pbs_submission_infos = dict(description='Hierarchical network, testing effect of M and M_layer_one. No filtering.', 
                            ...
                            other_options=dict(action_to_do='launcher_do_hierarchical_precision_M_sparsity_sigmaweight_feature_pbs', 
                                               ...
                                               num_repetitions=3, 
                                               label='hierarchical_M_sparsity_sigmaweight_volume_featurelayer'), 
                            walltime='10:00:00', 
                            memory='2gb', 
                            simul_out_dir=os.path.join(os.getcwd(), 'hierarchical_M_sparsity_sigmaweight_volume_featurelayer_210513'),
                            submit_label='hier_feat_new')

version='classic'
version='vary_threshold'
...
if version == 'vary_threshold':
    # Also vary the threshold now
    threshold_range                    =  dict(range=np.linspace(0.0, 10., 10.), dtype=float)
    dict_parameters_range['threshold'] =  threshold_range
