...
dataset_infos = dict(
    label=
    "Random sweeps for Thesis, chapter about Single item characterisation. HIERARCHICAL CODE. low-sparsity regime, where high threshold seems to make super efficient populations? sigma_weights is useless, vary sparsity and threshold.",
    files="%s/%s*.npy" %
    ...
    # limit_max_files=1,
    parameters=['threshold', 'sparsity'],
    variables_to_load=[
        'result_all_precisions', 'result_FI_rc_curv', 'result_em_fits'
    ],
    post_processing=None,
    save_output_filename='hier_params')
