...
run_label = 'hierarchical_weights_plambdatheta_1try_repetitions{num_repetitions}_270818'
...
pbs_submission_infos = dict(
    ...
    Varying parameters controlling the weights and threshold of neurons.

    ==> ACTUALLY, sigma_weights is useless! Don't vary it anymore...
    ''',
    ...
    other_options=dict(
        ...
        threshold=1.0,
        bic_K=5,
        session_id='hier_1_plambdatheta',
        M=100,
        sigmax=0.3,
    ...
    submit_label='hier_weights_1try',
    resource=resource,
    partition=partition,
    qos='auto')
...
dict_parameters_range = {
    ...
    'sigma_weights': dict(
        sampling_type='uniform', low=0., high=2., dtype=float),
    'sparsity': dict(sampling_type='uniform', low=0., high=1., dtype=float),
}
