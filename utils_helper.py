...
def array2string(array):
  ...
  if array.ndim == 2:
    ...
    return '  |  '.join([
        ', '.join([' '.join([str(it) for it in obj]) for obj in item])
        for item in array
    ])


def say_finished(text='Work complete',
                 additional_comment='',
                 email_failed=True):
  '''
...
def email_finished(text='Work complete',
                   to='loic.matthey@gatsby.ucl.ac.uk',
...
def remove_functions_dict(dict_input):
  ...
  return dict(
      (k, v) for (k, v) in dict_input.iteritems() if not is_function(v))
...
def convert_deltatime_str_to_seconds(deltatime_str):
  ...
  time_splitted_rev = np.array(
      [float(x) for x in deltatime_str.split(":")[::-1]])
...
def file_exists_new_shell(filename):
  ...
  pipeoutput = subprocess.Popen(
      "ls " + folder, shell=True, stdout=subprocess.PIPE)
...
def fix_M_ratioconj(M, ratio_conj):
  ...
  assert np.allclose(np.round(M_corrected * ratio_corrected),
                     M_conj_true), "Error in ratio here..."

  return M_corrected, ratio_corrected


def convert_Mratiohierarchical_Mlowerhigher(M, ratio_hierarchical):
  M_layer_two = np.asarray(np.round(ratio_hierarchical * M)).astype(int)
...
def fix_ratiohierarchical(M, ratio_hierarchical):
  ...
  M_layer_two_fixed = (
      M_layer_one - M_layer_one_fixed + M_layer_two).astype(int)
