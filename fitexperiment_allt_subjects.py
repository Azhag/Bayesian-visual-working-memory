...
class FitExperimentAllTSubject(FitExperimentAllT):
  ...
  def __init__(self, parameters={}, debug=True):
    ...
    assert (parameters['experiment_id'] in (
        'bays09', 'gorgo11')), "Check me for other datasets first!"
  ...
  def load_dataset(self):
    ...
    self.subject_space = self.experimental_dataset['data_subject_split'][
        'subjects_space']
    ...
    self.experiment_data_to_fit = self.experimental_dataset[
        'data_subject_split']['data_subject_nitems'][self.subject]
    self.T_space = self.experimental_dataset['data_subject_split'][
        'nitems_space']
    self.num_datapoints = int(self.experimental_dataset['data_subject_split'][
        'subject_smallestN'][self.subject])
  ...
  def get_em_fits_arrays(self):
    ...
    if self.em_fits_arrays is None:
      ...
      self.em_fits_arrays['mean'] = self.experimental_dataset[
          'em_fits_subjects_nitems_arrays'][subject_i].T
...
def test_fitexperiment_allt_subjects():
  ...
  results = fit_exp.apply_fct_datasets_allT(
      dict(fct=compute_metrics, parameters=dict()))
