...
# encoding: utf-8
"""
EM_circularmixture_allitems_kappafi.py
...
def initialise_parameters_random(N, K, nb_initialisations=10):
    ...
    for i in xrange(nb_initialisations):
        ...
        all_params.append((mixt_target, mixt_random, mixt_nontargets, resp_ik))
...
def initialise_parameters_fixed(N, K):
    ...
    resp_ik_fixed = [np.empty((N, K+1)), ]*len(mixt_random_fixed)

    return zip(mixt_target_fixed, mixt_random_fixed, mixt_nontargets_fixed, resp_ik_fixed)
